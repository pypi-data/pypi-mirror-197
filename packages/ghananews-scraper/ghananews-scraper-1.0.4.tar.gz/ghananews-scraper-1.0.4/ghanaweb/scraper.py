...
class GhanaWeb:
    ...
    def download(self, output_dir=None):
        """scrape data"""
        self.response = self.response or requests.request('GET', self.url, headers=headers)
        self.soup = self.soup or BeautifulSoup(self.response.text, 'html.parser')
        lst_pages = [a for a in self.soup.find('div', {'class': 'afcon-news list'}).find_all('a')]
        ...
        try:
            ...
            with open(os.path.join(output_dir, self.file_name + ".csv"), mode='w', newline='') as csv_file:
                ...
                for page in lst_pages:
                    ...
                    title = soup_page.find('h1', {'style': 'clear: both;'})
                    content = soup_page.find('p', {'style': 'clear:right'})
                    if title or content:
