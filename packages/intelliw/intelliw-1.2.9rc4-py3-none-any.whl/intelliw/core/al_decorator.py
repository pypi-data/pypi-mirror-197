...
def __push_model_to_cloud(reporter, model_path, index, is_checkpoint, kwargs):
    ...
    try:
        ...
        save_fn = config.INSTANCE_ID if config.INSTANCE_ID != "" else curkey_r
        ...
        logger.info(
            f"上传模型文件到{client_type}, 上传地址{uploader.service_url}, checkpoint模式：{is_checkpoint}")
        try:
            uploader.upload(outpath)

            logger.info(f"上传模型文件到{client_type}成功：{curkey}, checkpoint模式：{is_checkpoint}")
            if is_checkpoint:
            ...
            logger.info(f"上传模型文件到{client_type}失败: {err_info}, checkpoint模式：{is_checkpoint}")
