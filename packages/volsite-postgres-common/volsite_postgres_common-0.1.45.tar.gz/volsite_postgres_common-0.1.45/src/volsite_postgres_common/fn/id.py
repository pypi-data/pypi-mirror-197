...
jsonb_uuid_array_2_uuid_array: Final = (
    f" CREATE OR REPLACE FUNCTION "
    f" {CFn.jsonb_uuid_array_2_uuid_array} (JSONB) "
    f" RETURNS UUID[] "
    f" AS $$ "
    f"   SELECT array_agg((x))::UUID[] || ARRAY[]::UUID[] "
    f"      FROM {BFn.jsonb_array_elements_text}($1) t(x);"
    f" $$ "
    f" LANGUAGE SQL "
    f" IMMUTABLE;")
...
def insert_util_fn__id(conn):
    ...
    insert_function(jsonb_uuid_array_2_uuid_array, CFn.jsonb_uuid_array_2_uuid_array, conn)
