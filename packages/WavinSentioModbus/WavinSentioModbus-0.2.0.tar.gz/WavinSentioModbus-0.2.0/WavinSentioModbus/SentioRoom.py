...
class SentioRoom:
    ...
    def updateData(self):
        ...
        self.temperatureSetPoint = (self._api.readRegister(SentioRegisterMap.RoomTempSetpoint, _subIndex = self.index) / 100)
        self.calculatedDewPoint = (self._api.readRegister(SentioRegisterMap.RoomCalcDewPoint, _subIndex = self.index) / 100)
        if(self.calculatedDewPoint == Defaults.InvalidFP2_100):
            self.calculatedDewPoint = None
        self.floorTemp  = (self._api.readRegister(SentioRegisterMap.RoomFloorTempActual, _subIndex = self.index) / 100)
        if(self.floorTemp == Defaults.InvalidFP2_100):
            self.floorTemp = None
        self.airTemp   = (self._api.readRegister(SentioRegisterMap.RoomAirTempActual, _subIndex = self.index) / 100)
        self.humidity  = (self._api.readRegister(SentioRegisterMap.RoomRelHumidity, _subIndex = self.index) / 100)
