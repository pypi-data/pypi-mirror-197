...
def modified_rosin_rammler(d: Union[np.ndarray[float], pd.Series] = np.array([53.8, 38.1, 26.7, 18.8, 13.3]),
                           d50: float = 10,
                           m: float = 1.42) -> Union[np.ndarray[float], pd.Series]:
    ...
    if isinstance(d, pd.Series):
        mp = pd.Series(data=mp, index=d.index, name='wt_pct_passing')
    return mp
