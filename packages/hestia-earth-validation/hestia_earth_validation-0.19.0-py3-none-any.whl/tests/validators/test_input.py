...
from hestia_earth.validation.validators.input import (
    ...
    validate_animalFeed_requires_fate
)
...
def test_validate_animalFeed_requires_fate_valid():
    # no inputs should be valid
    assert validate_animalFeed_requires_fate({}, {}) is True

    with open(f"{fixtures_path}/input/animalFeed-fate/valid.json") as f:
        cycle = json.load(f)
    assert validate_animalFeed_requires_fate(cycle, cycle.get('site'), 'inputs') is True


def test_validate_animalFeed_requires_fate_invalid():
    with open(f"{fixtures_path}/input/animalFeed-fate/invalid.json") as f:
        cycle = json.load(f)
    assert validate_animalFeed_requires_fate(cycle, cycle.get('site'), 'inputs') == {
        'level': 'error',
        'dataPath': '.inputs[0].fate',
        'message': 'must specify the fate of the feed'
    }
