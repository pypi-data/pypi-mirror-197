from txp.common.models.firestore_models_client import FirestoreModelsQuery, FirestoreModelsClient
...
class AssetMetrics(TxpRelationalEntity):
    ...
    def update_metric_in_firestore(cls, new_values: "AssetMetrics", tenant_id: str):
        ...
        if not r[0]:
            ...
            db = firestore.Client()
            assets_col: firestore.CollectionReference = db.collection("asset_metrics").document(tenant_id).collection(
                "assets")
            assets_col.document(new_values.asset_id).set(
                new_values.get_dict()
            )
            logging.info(f"Asset Metrics Created in Firestore for {new_values.asset_id}")
        else:
            ...
            logging.info(f"New worked hours total for {new_values.asset_id} : {new_values.worked_hours} secs")
            db = firestore.Client()
            assets_col: firestore.CollectionReference = db.collection("asset_metrics").document(tenant_id).collection("assets")
            assets_col.document(new_values.asset_id).update(
                new_values.get_dict()
            )
            logging.info(f"Asset Metrics Updated in Firestore for {new_values.asset_id}")
