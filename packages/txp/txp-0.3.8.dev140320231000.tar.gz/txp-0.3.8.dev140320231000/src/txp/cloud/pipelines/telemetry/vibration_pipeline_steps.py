...
class ProcessMachineMetricsForVibrationRows(beam.DoFn):
    ...
    """
    def process(self, key_value: Tuple[str, List[Dict]]):
        k, v = key_value
        logging.info(f"Received window of {len(v)} packages for {k}")

        rows = sorted(
            v,
            lambda row: row['observation_timestamp'],
            reverse=True
        )

        newest_row = rows[1]

        logging.info(f"The newest row has observation "
                     f"timestamp: {datetime.datetime.fromtimestamp(newest_row['observation_timestamp'])}")
