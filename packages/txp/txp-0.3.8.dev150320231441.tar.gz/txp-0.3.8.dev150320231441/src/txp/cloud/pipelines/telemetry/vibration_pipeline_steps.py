...
class ProcessMachineMetricsForVibrationRows(beam.DoFn):
    """Updates the metrics for the asset id in the rows.
    ...
    def process(self, key_value):
        ...
        rows = sorted(
            v,
            key=lambda row: row['observation_timestamp'],
            reverse=True
        )
        ...
        asset_metrics = AssetMetrics.get_proto_class()(
            asset_type="Conveyor",
            last_seen=datetime.datetime.fromtimestamp(timestamp, tz=pytz.timezone('America/Mexico_City')).strftime(
                AssetMetrics.last_seen_format()
            ),
            rpm=rpm[0],
        ...
        yield (AssetMetrics.get_instance_from_proto(asset_metrics), rows[0]['tenant_id'])
    ...
    def _compute_total_new_worked_time(self, rows: List[Dict]) -> float:
        ...
        """
        observation_time_col = list(map(
            lambda d: d['observation_time_secs'], rows
        ))
        logging.info(f"SUMA {sum(observation_time_col)}")
        return float(sum(observation_time_col) / 3600)


class WriteMetricsToFirestore(beam.DoFn):
    def process(self, element, *args, **kwargs):
        AssetMetrics.update_metric_in_firestore(element[0], element[1])
