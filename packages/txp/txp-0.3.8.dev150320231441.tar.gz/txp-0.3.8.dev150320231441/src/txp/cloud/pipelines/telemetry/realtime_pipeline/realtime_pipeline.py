...
def run():
    ...
    with beam.Pipeline(options=pipeline_options) as p:
        ...
        signal_collection = pubsub_entry | "FromProtoToJson" >> beam.ParDo(
            FromProtoToJson()
        )
        ...
        ###########################################################################################
        # time_signal = signal_collection | "TimeProcessing" >> beam.ParDo(
        #     ts.TimeProcessing()
        # )
        # (
        #     time_signal
        #     | "WriteTimeToBigQuery"
        #     >> beam.ParDo(ts.WriteToBigQuery(), known_args.time_table)
        #     | "WriteTimeToPubSub"
        #     >> beam.io.WriteToPubSub(topic=model_serving_topic, with_attributes=False)
        # )
        # (
        #     time_signal
        #     | "TimeMetrics" >> beam.ParDo(ts.TimeMetrics())
        #     | "WriteTimeMetricsToBigQuery"
        #     >> beam.ParDo(ts.WriteToBigQuery(), known_args.time_metrics_table)
        #     | "WriteTimeMetricsToPubSub"
        #     >> beam.io.WriteToPubSub(topic=model_serving_topic, with_attributes=False)
        # )
        # fft_signal = signal_collection | "FftProcessing" >> beam.ParDo(
        #     ts.FftProcessing()
        # )
        # (
        #     fft_signal
        #     | "WriteFftToBigQuery"
        #     >> beam.ParDo(ts.WriteToBigQuery(), known_args.fft_table)
        #     | "WriteFftToPubSub"
        #     >> beam.io.WriteToPubSub(topic=model_serving_topic, with_attributes=False)
        # )
        # (
        ...
        #     | "WriteFftMetricsToBigQuery"
        #     >> beam.ParDo(ts.WriteToBigQuery(), known_args.fft_metrics_table)
        #     | "WriteFftMetricsToPubSub"
        #     >> beam.io.WriteToPubSub(topic=model_serving_topic, with_attributes=False)
        # )
        # psd_signal = signal_collection | "PsdProcessing" >> beam.ParDo(
        #     ts.PsdProcessing()
        # )
        # (
        #     psd_signal
        #     | "WritePsdToBigQuery"
        #     >> beam.ParDo(ts.WriteToBigQuery(), known_args.psd_table)
        #     | "WritePsdToPubSub"
        #     >> beam.io.WriteToPubSub(topic=model_serving_topic, with_attributes=False)
        # )
        # (
        ...
        #     | "WritePsdMetricsToBigQuery"
        #     >> beam.ParDo(ts.WriteToBigQuery(), known_args.psd_metrics_table)
        #     | "WritePsdMetricsToPubSub"
        #     >> beam.io.WriteToPubSub(topic=model_serving_topic, with_attributes=False)
        # )
        ...
        vibration_elements_window = (
            ...
            >> beam.ParDo(txp.cloud.pipelines.telemetry.vibration_pipeline_steps.WriteMetricsToFirestore())
        )
