...
def xss_scanner(url, secure = True, tor = False, crawl = "0", my_file = " ", parse = " ", delay = 1):
    mal_scripts = ["<p>The Silent</p>", "\\<p>The Silent</p>\\", "<div>The Silent</div>", "\\<div>The Silent</div>\\", "<script>alert('The Silent')</script>", "\\<script>alert('The Silent')</script>\\", "<script>prompt('The Silent')</script>", "\\<script>prompt('The Silent')</script>\\"]
    ...
    for links in my_result:
        ...
        try:
            ...
            form_activities = form_scanner(clean, secure, tor)
