...
On Linux, just copy `npbackup` executable to `/usr/local/bin` or install via pip with `pip install npbackup`.
Note, when using an pip copy, you'll also need to install tkinter via `yum install python-tkinter` or `apt install python3-tk`.
