...
#define PID_PATH ("/tmp/daemonizer.pid") 
...
static bool create_pid_file(char const *pid_path) {
    pid_t pid = getpid();
    char pid_num_str[20] = {'\0'};

    sprintf(pid_num_str, "%d\n", pid);
...
static void run_detached(PyObject *callback) {
    log_t *lg = NULL;

    if (!clear_signal_mask()) {
        error(lg, "ERROR: Unable to clear signal mask");
        return;
    }
    ...
    if (!close_non_standard_file_descriptors()) {
        error(lg, "ERROR: unable to close non standard fds");
        return;
    }


    if (!attach_standard_file_descriptors_to_null()) {
        error(lg, "ERROR: Cannot attach standard fds to null");
        return;
    }
    ...
    if (!create_pid_file(PID_PATH)) {
        error(lg, "ERROR: unable to create pid file: %m\n"); // errno
        return;
    }
    ...
    if (!result) {
        error(lg, "unable to call function object");
    }
...
PyObject *detach(PyObject *unused, PyObject *args) {
    ...
    if (!PyArg_ParseTuple(args, "O", &callback)) {
        PyErr_SetString(PyExc_TypeError, "daemonize: incorrect arguments");
        return NULL;
    }

    if (!PyCallable_Check(callback)) {
        PyErr_SetString(PyExc_TypeError, "daemonize: a function is required");
        return NULL;
    }

    pid_t pid = fork(); // from this point on (parent and child) are running
    if (pid < 0) {
        PyErr_SetString(PyExc_OSError, "daemonize: initial fork failed");
        return NULL;
    }
