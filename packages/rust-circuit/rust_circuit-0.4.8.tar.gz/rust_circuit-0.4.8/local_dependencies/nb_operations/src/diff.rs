...
use anyhow::Result;
use circuit_base::{CircuitNode, CircuitNodeSelfOnlyHash, CircuitRc};
...
pub fn diff_circuits(
    ...
    fn recurse(
        ...
        if new == old {
            ...
            let child_printed = new_options.repr(new)?;
            let child_printed =
            ...
            result.push('\n');
            seen_diffs.insert(diffkey, idstr);
            return Ok(());
        }
        ...
        result.push_str(&indent(new_options.repr(new)?, last_child_stack.len() * 2));
        result.push('\n');

        let mut new_options = options.clone();
        new_options.colorer = Some(PrintOptions::fixed_color(removed_color));
        result.push_str(&indent(new_options.repr(old)?, last_child_stack.len() * 2));
        result.push('\n');
