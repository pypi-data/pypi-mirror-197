...
from anylearn.sdk import Artifact, DatasetArtifact, ModelArtifact, get_task_output
...
def quick_train(algorithm_id: Optional[str]=None,
    ...
    if algorithm_dir:
        ...
        artifact_full_names = collector.collect(algorithm_dir)
        artifacts.extend([
            DatasetArtifact.from_full_name(_fn)
            for _fn in artifact_full_names['datasets']
        ])
        artifacts.extend([
            ModelArtifact.from_full_name(_fn)
            for _fn in artifact_full_names['models']
        ])
        artifacts.extend([
            get_task_output(_id)
            for _id in artifact_full_names['task_outputs']
        ])
