...
GITHUB_BUTTON="""<a href="https://github.com/uncenter/chemic" target"_blank" type="button" class="text-white bg-[#24292F] hover:bg-[#24292F]/90 focus:ring-4 focus:outline-none focus:ring-[#24292F]/50 font-medium rounded-lg text-sm px-5 py-2.5 text-center inline-flex items-center dark:focus:ring-gray-500 dark:hover:bg-[#050708]/30 mr-2 mb-2">
...
def gui():
    ...
    def periodic_table():
        def result(data):
            ...
            if (
                ...
                molecule = Formula(molecule)
                with bottom_content:
                    result_box = ui.card().classes("w-2/3")
                    with result_box:
                        if molecule.name != "Unknown":
                            ui.label(f"{molecule.name}")
                        ui.label(f"{molecule.mass} g/mol")
    ...
    with ui.header().classes(replace="row justify-between h-10"):
