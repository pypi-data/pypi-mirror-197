...
def violinplot_facetgrid(entropy_report, fig_size = (20,20)):
    g = sns.FacetGrid(entropy_report,  col="Diagnosis", col_wrap=4, sharey=False ,  sharex=False)
    # Map histogram plot to "total_bill" column
    g.map(sns.violinplot, "dataset", "entropy",  palette="Set1", order = entropy_report['dataset'].value_counts().sort_index(ascending = False).index.values.tolist())
    g.set_titles(col_template="{col_name}")
    g.set_ylabels("Entropy")
    g.fig.set_size_inches(fig_size[0],fig_size[1])
    g.tight_layout()
...
def entropy_plot(entropy_report, type_report = 'summary', type_plot = 'histogram', fig_size = (20,20), font_scale = 1 , **kwargs):
    """
