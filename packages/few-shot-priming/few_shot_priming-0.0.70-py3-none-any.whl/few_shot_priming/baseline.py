...
def run_experiment_baseline(splits, offline=True, validate=False, params=None):
    ...
    train_dataset = Dataset(splits["training"], {"PRO": 1, "CON": 0})
    if validate:
        experiment_type = "validate"
        test_dataset = Dataset(splits["validation"], {"PRO": 1, "CON": 0})
    else:
        experiment_type = "test"
        test_dataset = Dataset(splits["test"], {"PRO": 1, "CON": 0})

    train_dataloader = DataLoader(train_dataset, batch_size=params["batch-size"], shuffle=True, path=path)
    test_dataloader = DataLoader(test_dataset, batch_size=params["batch-size"], shuffle=True, path=path)
