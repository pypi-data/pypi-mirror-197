...
class FlanT5BaseConfig(FinetuningConfig):
    ...
    **Examples:**
    ...
    Args:
        ...
        lr_scheduler_type:
            Type of learning rate scheduler to use (options are "linear", "cosine",
        ...
        optimizer:
            Optimizer to use for training (options are "adamw_hf", "adamw_torch",
            "adamw_apex_fused", "adamw_anyprecision" or "adafactor")
        label_smoothing_factor:
            The label smoothing factor to use. Zero means no label smoothing,
            otherwise the underlying onehot-encoded labels are changed from 0s
