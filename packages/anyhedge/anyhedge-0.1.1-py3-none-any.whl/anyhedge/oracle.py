...
class StrictPricepoints:
    ...
    def new(cls, pricepoints: Sequence[Pricepoint]):
        sorted_pricepoints = cls._sort_pricepoints(pricepoints)
    ...
    def _validate_pricepoints(cls, pricepoints: Sequence[Pricepoint]):
        ...
        for p in pricepoints:
            ...
            if existing is None:
                ...
                if p == existing:
                    raise ValueError(f'duplicate pricepoint in sequence: {p}')
                else:
                    raise ValueError(f'same timestamp, same oracle, different value: {p} vs {existing}')

        # enforce sorting. not efficient but performance is not yet an issue
        sorted_pricepoints = cls._sort_pricepoints(pricepoints)
        validators.equal(tuple(pricepoints), sorted_pricepoints)
