...
def build_docker_image(
    ...
    dockercfg_path = os.path.join(home_dir, ".dockercfg")
    try:
        client = _get_docker_client()
        try:
            if os.path.exists(docker_config_path):
                logger.info(f"Found {docker_config_path!r}, trying to login with that")
                client.login(reauth=True, dockercfg_path=dockercfg_path)
            elif os.path.exists(dockercfg_path):
                logger.info(f"Found {dockercfg_path!r}, trying to login with that")
                client.login(reauth=True, dockercfg_path=dockercfg_path)
        except Exception:
            logger.warning(
                "Failed to login with docker config files, proceeding ahead anyway ..."
            )
