    ...
    if (
        ...
        if component.image.build_source.local_build:
            if not env_has_docker():
                logger.info(
                    "We did not find Docker locally installed on this system. So we will be building remotely. However, the builds can be a bit slow. If you want faster builds, please install Docker locally."
                )
                local_build = False
            else:
                logger.info(
                    "We will build the image using the local docker installed. This should give you the fastest performance and builds and is in general recommended. If you don't want to build using local docker and instead build remotely, please set `image.build_source.local_build` to `false` in your manifest file or code."
                )
                local_build = True
        else:
            logger.info(
                "we will be building remotely. However, the builds can be a bit slow. If you want faster builds, please install docker locally"
            )
            local_build = False
