...
class JSONRPCServer(ContextableService, AbstractRPCCompatible):
    ...
    async def _execute_single(
        ...
        self.logger.info('<-', request=body, response=result)
    ...
    async def _execute_batch(
        ...
        _request = [request[0]]
        self.logger.info('->', request=_request, batch=True)
        results = []
        ...
        self.logger.info('<-', request=[request[0]], response=[results[0]], batch=True)
        session = self.get_session()
        if self._sessions and session and session.stored and session.changed:
            await self._sessions.save_session(session)
