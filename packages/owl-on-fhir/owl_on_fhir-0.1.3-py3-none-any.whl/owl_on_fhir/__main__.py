...
def owl_to_fhir(
    input_path_or_url: str, out_dir: str = OUTDIR, out_filename: str = None, include_only_critical_predicates=False,
    ...
    # out_dir: If they don't specify it, use input path if they not passed a URL, otherwise where program called from
    out_dir = out_dir if out_dir else os.path.dirname(input_path) if not url else os.getcwd()
    # TODO: fix missing outdir
...
def cli():
    ...
    parser.add_argument(
        ...
        help='If present, will run all favorite ontologies found in pre-baked config variable `FAVORITE_ONTOLOGIES` in'
             '`owl_on_fhir/__main__.py`. . If using `--favorites`, the other CLI flags are not relevant. Instead, you '
             'can customize by editing `FAVORITE_DEFAULTS` if you are running a clone of the project. Presently, the '
             'configure these favorites is not possible if running the installation from pip / PyPi.')
    ...
    if d['favorites']:
        ...
        del d['favorites']
        # Validate
        if not d['input_path_or_url']:
            print('Missing required param `--input-path-or-url`. Must specify an input path or URL.', file=sys.stderr)
        # Run
        else:
            owl_to_fhir(**d)
