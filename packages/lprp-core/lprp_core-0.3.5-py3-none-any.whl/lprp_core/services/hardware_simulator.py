...
class HardwareSimulator(Service):
    ...
    async def update_gate_state(self, value: GateState) -> None:
        response = requests.put(f"{self.address}/{self.identity}/gate_state", json={"gate_state": value})
        return
    ...
    async def update_visual_indicator_state(self, value: VisualIndicatorState) -> None:
        response = requests.put(f"{self.address}/{self.identity}/visual_indicator_state", json={"visual_indicator_state": value})
        return
