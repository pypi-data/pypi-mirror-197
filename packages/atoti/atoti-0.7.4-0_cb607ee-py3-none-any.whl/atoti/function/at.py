...
def at(
    ...
    coordinates: Union[
        Condition[
            LevelCoordinates,
            Literal["eq"],
            Union[Constant, LevelCoordinates],
            Optional[Literal["and"]],
        ],
        Mapping[
            Level,
            Union[ConstantValue, Level],
        ],
    ],
    /,
) -> MeasureDescription:
    ...
    if isinstance(coordinates, Mapping):
        ...
        coordinates = combine_conditions(
            (
                [
                    ComparisonCondition(
                        subject=level._coordinates,
                        operator="eq",
                        target=convert_to_operand(target),
                    )
                    for level, target in coordinates.items()
                ],
            )
        )
    ...
    for subject, target in condition_to_dict(coordinates).items():
        ...
        if isinstance(target, LevelCoordinates):
