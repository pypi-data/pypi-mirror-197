...
def load_files(filelist: Union[list[Path],list[str]]) -> Spectra:
    ...
    if len(filelist) > 1:
        for i_file, file in enumerate(filelist[1:], start=1):
            ...
            assert np.all(x == wl), f"Can only merge spectra with identical wavelength axis (current: '{str(file)}'"
            assert np.all(o == order), f"Can only merge spectra with identical orders (current: '{str(file)}'"
            Y[:,i_file] = y
            head.append(h)
