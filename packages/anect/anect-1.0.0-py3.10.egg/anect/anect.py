...
def _anec_get_name(name, hwxpath):
	if (not name):
...
def anec_convert(hwxpath, name="model", force=False):
	res = dotdict({"path": hwxpath})
	res.name = _anec_get_name(name, hwxpath)
	...
	res["nchw"] = _anec_get_nchw(hwxpath)
...
if __name__ == '__main__':
	parser = argparse.ArgumentParser(description='ane converter')
	parser.add_argument('hwxpath', type=str)
	...
	if (os.path.splitext(args.hwxpath)[1] == ".mlmodel"):
		logger.info("warning: pass the hwx output of coreml2hwx, not the mlmodel.")

	res = anec_convert(args.hwxpath, args.name, args.force)
	if (args.print):
		anec_print(res)

	if (args.struct or args.all):
		anec_whdr(res, prefix=args.outdir)

	if (args.data or args.all):
		anec_wbin(res, prefix=args.outdir)
