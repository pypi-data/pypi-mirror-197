...
class OwlModel:
    ...
    def get_classes_with_label(self):
        ...
        list_classes_label_sparql = list(
            self.__ontology.world.sparql(constants.SPARQL_LABEL_EXTRACT))
        for class_with_label in list_classes_label_sparql:
            if not str(class_with_label).startswith(constants.NS_OWL):
                list_classes_with_label.append(class_with_label)
