...
class wavefa3:
    ...
    def  __init__(self,filen_in,printlevel=0):
        ...
        try:
          ...
          self.cdepNN=np.array(1)
          self.evenNN=np.array(0)
          self.j12NNmax=np.array(-1)
          self.l12NNmax=np.array(-1)
          self.alphaNNcdepmax=np.array(0)
          self.alphaNNmax=np.array(0)
          self.mt12NNmax=np.array(-10)
          self.mt12NNmin=np.array(-10)
          self.qnalphaNN=np.array([])
        ...
        try:
          ...
          self.alpha_fein=np.array(1.0/137.035999074)
        try:  
          ...
          self.m_eta=547.862/self.hbarc  
          self.m_kaon0=497.614/self.hbarc
          self.m_kaonp=493.677/self.hbarc
          self.m_kaon=(self.m_kaonp+self.m_kaon0)/2.0
          self.m_omega=782.65/self.hbarc
          self.m_rho=775.26/self.hbarc
        ...
        try:
          ...
          self.mnuc=2*self.mneu*self.mprot/(self.mneu+self.mprot)  
        ...
        try:
          ...
          self.msigave=(self.msigp+self.msig0+self.msigm)/3.0
        try:  
          self.mxi0=group['mxi0'][()]
          self.mxim=group['mxim'][()]
          self.mxiave=group['mxiave'][()]
        except:
          self.mxi0=1314.86/self.hbarc
          self.mxim=1321.71/self.hbarc
          self.mxiave=(self.mxim+self.mxi0)/2.0
