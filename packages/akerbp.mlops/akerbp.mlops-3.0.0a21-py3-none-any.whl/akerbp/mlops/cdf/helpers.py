...
env_vars = config.read_env_vars()
env_vars.platform = "cdf"
...
def create_function_from_folder(
    ...
    try:
        ...
        logger.debug(f"Environment variables: {envs}")
        logger.info(
...
def create_function_from_file(
    ...
    try:
        ...
        logger.info(
            f"Created function {human_readable_name} with external id {function_name}: {file_id=}, {envs=}"
        )
