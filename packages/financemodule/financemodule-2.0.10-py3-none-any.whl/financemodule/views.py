...
class FinanceModuleHandler:
    ...
    def currency_conversion(self , from_currency , from_amount , to_currency , from_rate = None, to_rate = None ,exch_rate = None) -> None:
        ...
        if from_currency and to_currency != self.base_currency:
            ...
            if not exch_rate:
                exch_rate = from_rate
            to_amount = from_amount * exch_rate
        return round(to_amount) , round(exch_rate)
    ...
    def repayment(self):
        ...
        # credited_amount = FinanceAccounting.objects.get(finance_model = obj , contract_ref = self.finance_req_id ,
        #             interest_paid_by = self.interest_paid_by , type = AccountTypeChoice.CREDIT , account_type = UserTypeChoices.CUSTOMER ).latest()
