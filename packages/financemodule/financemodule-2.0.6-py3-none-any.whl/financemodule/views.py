...
class FinanceModuleHandler:
    ...
    def calculated_interest_values(self):
        ...
        if self.model_type == ModelTypeChoices.FINANCING or ModelTypeChoices.REPAYMENT  and self.program_type == ProgramTypeChoices.APF :
            ...
            if Interestaccount.objects.filter(program_type=self.program_type , currency=self.finance_currency).exists():
                return self.calculated_interest_amount(amount=round(self.finance_amount))[0], self.finance_currency 
            # base_currency 
            elif Interestaccount.objects.filter(program_type=self.program_type , currency=self.base_currency).exists():
                return self.calculated_interest_amount(amount=round(self.finance_amount))[0] * exchange_rate_queryset.rate_mid  , self.base_currency
        else:
    ...
    def create_finance(self):
        ...
        current_interest_amount = self.calculated_interest_amount(amount=round(self.finance_amount))

        
        calculated_amount = round(self.finance_amount) - current_interest_amount[0]
        ...
        finance_model = Finance.objects.create(
                ...
                finance_amount = round(self.finance_amount),
        ...
        if self.interest_paid_by == InterestPaidByChoices.OWNPARTY:
            ...
            currency_conversion_remittance = self.currency_conversion(self.finance_currency, round(self.finance_amount), self.account_info_2['currency'] )[0]
            ...
            amount = round(self.finance_amount) , 
            account = self.gets_loan_account().account , 
            account_type= UserTypeChoices.CUSTOMER,
            base_currency= self.base_currency ,
            base_currency_amount = self.base_currency_calculation(self.finance_currency ,round(self.finance_amount) , exchange_rate_queryset_1.rate_mid)[0] ,
            exch_rate = self.base_currency_calculation(self.finance_currency , round(self.finance_amount) , exchange_rate_queryset_1.rate_mid)[1] 
            ),
        ...
        if self.interest_paid_by == InterestPaidByChoices.COUNTERPARTY:
            ...
            account_type= UserTypeChoices.CUSTOMER,
            base_currency= self.base_currency,
            base_currency_amount=self.base_currency_calculation(self.account_info_2['currency'] , round(final_amount[0]) , exchange_rate_queryset_2.rate_mid)[0] ,
            exch_rate = self.base_currency_calculation(self.account_info_2['currency'], round(final_amount[0]) , exchange_rate_queryset_2.rate_mid)[1] 
            ),
    ...
    def repayment(self):
        ...
        # if (round(self.finance_amount) - credited_amount.amount) > 0:
        #     status = "PARTIALLY SETTLED"
        # else:
        #     status = "SETTLED"
        return current_interest_amount[0] , current_interest_amount[1] , obj.finance_date.date().isoformat() , obj.id , status
