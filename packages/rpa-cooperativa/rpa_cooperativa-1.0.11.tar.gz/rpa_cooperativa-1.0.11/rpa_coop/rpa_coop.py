...
class Dados:
    ...
    def criar_engine(self, db:str, user_db='user_opcional', password_db='senha_opcional', ip_ou_host_db='ip_host_opcional', porta='3306', library_sql='mysqlconnector'):
        ...
        '''
        if db=='planning' or db=='fluid' or db=='cronos' or 'controles_internos' or 'bok_credito' or 'pufv' or 'boas_vindas':
            ...
            porta = 9996
            return create_engine(f'denodo://{self.user_denodo}:{self.pw_denodo}@{self.ip_denodo}:{porta}/{db}')
        elif db == 'grafana':
            ...
            porta = 3307
            return mysql.connect(host=self.ip_sistema_senhas, port=porta, database=banco, user=self.user_sistema_senhas, password=self.pw_bd_sistema_senhas)
        else:
            return create_engine(f'mysql+{library_sql}://{user_db}:{password_db}@{ip_ou_host_db}:{porta}/{db}')
    ...
    def consultar_banco_dados(self, conexao_engine, query_sql:str, retorna_DataFrame=True):
        ...
        if retorna_DataFrame:
            ...
            rows = conexao_engine.execute(query_sql)
            return rows
