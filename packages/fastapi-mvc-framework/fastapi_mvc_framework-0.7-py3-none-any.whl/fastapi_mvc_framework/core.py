    ...
    def _wapper(targetController):  
        ...
        class puppetController( targetController ,_controllerBase ): 
            ...
            async def _constructor(self,request = Request,response=Response):  
                ...
                elif not hasattr(request,'params'):
                    request.params = {}
...
def run(*args,**kwargs): 
    ip =  "host" in kwargs  and kwargs["host"]  or '0.0.0.0' 
    ...
    if isDebug:
        ...
        for router in all_routers:
            for r in router.routes:
                ...
                print(f"*****   \033[1;43m  http:{host}:{port}{r.path} \033[0m ->({funcname})" )
    
    uvicorn.run(__app, host=ip, port=port)