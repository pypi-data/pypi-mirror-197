...
impl EtalonProp {
    ...
    fn new() -> EtalonProp {
        ...
        let intr = vec![0.0, rp, 100.0];
        let intt = vec![0.0, 2.0 * PI, 100.0];
        let theta3: f64 = 0.0;
...
fn simpsolve<F: Fn(f64,f64,&EtalonProp,f64,f64,f64,f64,f64) -> Complex<f64>>
    ...
    // println!("stepsss in simpsolve { } { } { } { } { } { } ", startx,stepsx,endx,starty,stepsy,endy);
    // println!("inputt  in simpsolve { } { } { } { } { } { } { } { }", xi,eta,xi,eta,e_p.f,2.0 * PI / wvl,wvl,e_p.theta3);

    for i in 0..stepsx+1 {
        let x = startx + stepx * i as f64;
        for j in 0..stepsy+1 {
            let y = starty + stepy * j as f64;
            result += function(x, y, e_p, xi, eta, xi0, eta0, wvl) *  wsipm(i,j,stepsx,stepsy) ;
        }
    }

    // result.re *= stepx * stepy / 9.0;
    // result.im *= stepx * stepy / 9.0;
    result * stepx * stepy / 9.0

    // println!("stepsss in simpsolve {:?} {:?} {:?}", wvl,result.re , result.im );
    // result.iter().map(|x| (x * x.conj()).powf(2.0).re).collect()
...
fn integrate(e_p: &EtalonProp, waveaxis: Vec<f64>, x:f64, y:f64, tiltx:f64, tilty:f64, teldegree:f64) -> Vec<f64> { //Vec<Complex<f64>> {//Vec<f64> {
    ...
    result.iter().map(|x| (x * x.conj()).re).collect()
        // output
