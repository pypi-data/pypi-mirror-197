...
class KubernetesJobOperator(KubernetesJobOperatorDefaultsBase):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.image = image
        logging.info(kwargs.get('params').__str__())
        self.envs = kwargs.get('params')
    ...
    def _to_kubernetes_env_list(cls, envs: dict):
        logging.info("envs:  "+envs.__str__())
        env: dict = envs.get('env')
        logging.info("env: " + env.__str__())
        task_env = env.get(cls.task_id)
        logging.info("task_env:  " + task_env.__str__())
        return [{"name": k, "value": f"{task_env[k]}"} for k in task_env.keys()]
