...
def get_data(code, flags=False, **kwargs):
    ...
    opt = ["filter_pars", "verbose", "reverse_time", "optim"]
    ...
    is_optim = kwargs.get("optim", False)
    ...
    assert type(is_optim) is bool, "Error: 'optim' must be a boolean."
    if is_optim:
        assert all(par in filter_pars for par in ["startPeriod", "endPeriod"]),\
            "Error: 'optim=True' needs both 'startPeriod' and 'endPeriod' in filter_pars."
    ...
    if filter_pars == dict():
        ...
        if is_optim:
            par_dict = {}
            for par in get_pars(code):
                if par in nontime_pars.keys():
                    par_dict[par] = len(nontime_pars[par])
                    exp_row *= par_dict[par]
                else:
                    exp_row *= len(get_par_values(code, par))
            exp_row *= len(period_range(s,e))
            
            if exp_row < 1e6:
                nontime_pars = ["+".join(nontime_pars[par]) for par in nontime_pars]
            else:
                np = (exp_row - 1) / 1e6
                par_sorted = sorted(par_dict.values(), reverse=True)
                compl = []
                mult = []
                for l in range(1, len(par_sorted) + 1):
                    for subset in combinations(par_sorted, l):
                        mul = 1
                        for s in subset:
                            mul *= s
                        diff = mul - np 
                        if diff > 0:       
                            ...
                            mult.append(diff)
                if mult != []:
                    optidx = mult.index(min(mult))
                    optimal = list(compl[optidx])
                    nontime_pars = ["+".join(nontime_pars[par]) if par not in optimal 
                                        else nontime_pars[par] for par in nontime_pars]
