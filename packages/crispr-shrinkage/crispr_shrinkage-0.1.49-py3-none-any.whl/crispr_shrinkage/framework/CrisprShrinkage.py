...
class StatisticalHelperMethods:
    ...
    def calculate_interval_prob(samples: List[float], interval: Tuple[float, float]) -> float:
        return sum(((samples >= interval[0]) and (samples <= interval[1]))) / len(samples)
...
def perform_adjustment(
    ...
    def set_prob_null_interval(guide_set: List[Guide]) -> List[Guide]:
        for guide in guide_set:
            LFC_estimate_combined_prob_null_interval_rescaled: float = calculate_interval_prob(guide.guide_count_posterior_LFC_samples_normalized_average_rescaled, LFC_rescaled_null_interval)
            LFC_estimate_combined_prob_null_interval: float = calculate_interval_prob(guide.guide_count_posterior_LFC_samples_normalized_average, LFC_null_interval)
            LFC_estimate_per_replicate_prob_null_interval_rescaled = np.asarray([calculate_interval_prob(guide.guide_count_posterior_LFC_samples_normalized_list_rescaled[rep_i], LFC_rep_rescaled_null_interval) for rep_i in replicate_indices])
            LFC_estimate_per_replicate_prob_null_interval = np.asarray([calculate_interval_prob(guide.guide_count_posterior_LFC_samples_normalized_list[rep_i], LFC_rep_null_interval) for rep_i in replicate_indices])
