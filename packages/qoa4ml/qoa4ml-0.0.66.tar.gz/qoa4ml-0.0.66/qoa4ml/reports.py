...
class Qoa_Client(object):
    ...
    def ex_observe_accuracy(self, value, accuracy, dependency=[]):
        if "prediction" not in self.ex_report:
            self.ex_report["prediction"] = {}
        pre_uuid = str(uuid.uuid4())
        predictions = {}
        if dependency:
            ...
            source = []
        predictions[pre_uuid] = {"value": value, "accuracy": accuracy, "source":source, "qoa_id": self.qoa_uuid}
        self.ex_report["prediction"].update(predictions)
        return predictions
    ...
    def ex_reset(self):
        self.ex_report = {"execution_graph":{"instances":{},"last_instance":[]},"data_quality":{}, "performance":{self.cur_stage_id:{"response_time": {}}}}
        self.report_list = []
