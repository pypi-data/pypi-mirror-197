...
def _kmeans_single_elkan(
    ...
    for i in range(max_iter):
        ...
        if np.array_equal(labels, labels_old):
            ...
            if callback_on_iteration_end is not None:
                inertia = _inertia(X, sample_weight, centers, labels, n_threads)
                callback_on_iteration_end(i, True, inertia, labels)  # last iteration
            break
        else:
            # No strict convergence, check for tol based convergence.
            if callback_on_iteration_end is not None:
                inertia = _inertia(X, sample_weight, centers, labels, n_threads)
                callback_on_iteration_end(i, False, inertia, labels)
    ...
    if not strict_convergence:
        ...
        if callback_on_iteration_end is not None:
            inertia = _inertia(X, sample_weight, centers, labels, n_threads)
            callback_on_iteration_end(i+1, True, inertia, labels)
...
class KMeansPartialResult(Bunch):
    def __init__(self, iteration, is_last, inertia, labels):
        super().__init__(iteration=iteration, is_last=is_last, inertia=inertia, labels=labels)
...
class KMeans(_BaseKMeans):
    ...
    def fit(self, X, y=None, sample_weight=None, on_partial_result=None):
        ...
        def callback_on_iteration_end(iteration, is_last, inertia, labels):
            if on_partial_result is not None:
                r = KMeansPartialResult(iteration, is_last, inertia, labels)
                on_partial_result(r)
