...
sheet[mc.年龄>11]['姓名'][mg.allColumns][:]
_ = sheet
_ = _[mc.年龄>=12]  # 比较
...
from coolmongo import MongoColumn
try:
    from coolmongo._coolmongo import dbORM, sheetORM
except:
    from coolmongo.coolmongo._coolmongo import dbORM, sheetORM


mkconn = lambda: MongoClient(host='localhost', port=27017)
orm = mg.ORM(mkconn=mkconn)

del orm['_test_test_test_']
x = len(orm)
orm['_test_test_test_']['test'] + {'a':1}
assert len(orm) - x == 1
del orm['_test_test_test_']
assert len(orm) - x == 0
...
del orm['_test_test_test_2']
#################################################### ORM

# 调用conn原生方法和属性
'admin' in orm.list_database_names()


# 调用db原生方法和属性
assert db.list_collection_names() == []

# 判断存在表、获取sheetORM、删除表、len
db['sunshine1'] + dict(a=10)
db['sunshine2'] + dict(a=10)
db['sunshine3'] + dict(a=10)
db['sunshine4'] + dict(a=10)
db['sunshine5'] + dict(a=10)
db['sunshine6'] + dict(a=10)
assert len(db) == 6
assert len(db[:]) == 6
assert 'sunshine1' in db
del db['sunshine1']
del db['sunshine2', 'sunshine3']
assert len(db) == 3
assert 'sunshine1' not in orm
assert 'sunshine2' not in orm
assert type(db['sunshine4']) is sheetORM
x = db['sunshine5', 'sunshine6']
assert type(x) is list and len(x) == 2 and type(x[0]) is sheetORM
del db[:]
assert len(db[:]) == 0
...
assert type(sheet) is type(sheet2)
del db['学生2']
...
def 重置数据():
    ...
    r1 = sheet + line1
    assert len(sheet) == 1
    r2 = sheet + [line2, line3, line4, line5, line6, line7, line8, line9]
    assert len(sheet) == 9
    r1.inserted_id
    r2.inserted_ids
...
r = sheet[mc.性别=='男'][mc.爱好==mg.containAll('编程','跑步')][:]
assert r
for x in r:
    assert x['视力'] == 4.5
    assert x['身高'] == 172
...
sheet[mc.序号>=6][:] = None
assert len(sheet) == len(sheet[:]) == 5
sheet[:] = None
assert len(sheet) == len(sheet[:]) == 0


# 成员运算
重置数据()

r = sheet[mc.幸运数字==mg.containAll(2,3,4)][:]
assert len(r) == 1
assert r[0]['序号'] == 2
assert len(sheet[mc.幸运数字==mg.containAll()][:]) == 9

r = sheet[mc.幸运数字==mg.containAny(2)][:]
assert len(r) == 2
assert r[0]['序号'] == 1
assert not sheet[mc.幸运数字==mg.containAny()][:]

r = sheet[mc.幸运数字==mg.containNo(1,2,3)][:]
assert len(r) == 6
assert r[0]['序号'] == 4
assert len(sheet[mc.幸运数字==mg.containNo()][:]) == 9

r = sheet[mc.序号==mg.isin(4,5,6)][:]
assert len(r) == 3
assert r[0]['序号'] == 4
assert r[-1]['序号'] == 6
assert len(sheet[mc.序号==mg.isin()][:]) == 0

r = sheet[mc.序号==mg.notin(4,5,6)][:]
assert len(r) == 6
assert len(sheet[mc.序号==mg.notin()][:]) == 9

assert len(sheet[mc.姓名==mg.re('小')][:]) == 9

# 集合运算

r = sheet[ mc.序号>=3 ][ mc.序号<=7 ][:]
assert len(r) == 5
assert r[0]['序号'] == 3
...
assert len(sheet[2:8]) == len(sheet[8:2]) == len(sheet[2:-2]) == len(sheet[-2:2]) == len(sheet[-8:8]) == len(sheet[8:-8]) == 7
assert len(sheet[-2:-8]) == len(sheet[-8:-2]) == 7

assert len(sheet[1:9]) == len(sheet[9:1]) == len(sheet[1:-1]) == len(sheet[-1:1]) == len(sheet[-9:9]) == len(sheet[9:-9]) == 9
assert len(sheet[-1:-9]) == len(sheet[-9:-1]) == 9
assert len(sheet[:]) == len(sheet[:9]) == len(sheet[:-1]) == 9
...
r = sheet['姓名', '成绩']['序号'][mg.allColumns][:]
...
# 排序
r = sheet.order(序号=False)[:]
assert len(r) == 9
assert r[0]['序号'] == 9

r = sheet.order(序号=False)[9:1:2]
assert len(r) == 5
assert r[0]['序号'] == 1
...
重置数据()
assert len(sheet) == 9
assert len(sheet[mc.序号<=4]) == 4
assert '_test_test_test_' in orm
assert '学生' in db
assert len(db) == 1
orm.getDbNames()
assert db.getSheetNames() == ['学生']
...
del orm['_test_test_test_']
assert '_test_test_test_' not in orm
