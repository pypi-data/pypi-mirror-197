...
class EuropeanOptionPricingClassical:
    """
    This class implements the classical methods to solve the European call/put option pricing problem.
    The classical solver includes the Black-Scholes-Merton (BSM) model and classical Monte Carlo (CMC) method.
    """

    def __init__(self, s0: float = 1.0, k: float = 1.0, t: float = 1, r: float = 0.05, sigma: float = 0.2, cp:str = 'call', ls:str = 'long') -> None:
        """
        :param s0: initial stock price
        :param k: strike price
        :param t: time to maturity, in years
        :param r: risk-free annualized interest rate
        :param sigma: volatility
        :param cp: call or put
        :param ls: long or short
        """
        self.s0_ = s0
        self.k_ = k
        self.t_ = t
    ...
    def set_pricing_method(self, method: str) -> None:
        """
        :param method: the method to solve the option pricing problem
        """
    ...
    def set_CMC_sample_size(self, sample_size: int) -> None:
        """
        :param sample_size: the sample size of the classical Monte Carlo method
        """
        if sample_size <= 0:
    ...
    def long_call_price_CMC(self) -> float:
        ...
        The stock price at maturity (sT) is generated by the geometric Brownian motion, and follows a log-normal distribution.
        The payoff is max(sT - k, 0), where k is the strike price.
        The discounted payoff is exp(-r * t) * max(sT - k, 0).
        """
        z = np.random.normal(0, 1, self.CMC_sample_size_)
        sT = self.s0 * np.exp((self.r - 0.5 * self.sigma ** 2) * self.t + self.sigma * np.sqrt(self.t) * z)
        # return np.exp(-1 * self.r * self.t) * np.mean(np.maximum(sT - self.k, 0))
    ...
    def bsm_price(self) -> float:
        """
        :return: the price of the European call/put option, using Black-Scholes-Merton formula directly
        """
    ...
    def cmc_price(self) -> float:
        """
        :return: the price of the European call/put option, using classical Monte Carlo method
        """
        method_backup = self.pricing_method_
    ...
    def compare_prices(self) -> None:
        """
        Compare the prices of the European call/put option, using BSM and CMC.
        """
        method_backup = self.pricing_method_
        self.pricing_method_ = 'BSM'
    ...
    def long_call_delta_BSM(self) -> float:
        ...
        delta = dC / ds = N(d1), where d1 = (ln(s0 / k) + (r + 0.5 * sigma ** 2) * t) / (sigma * sqrt(t))
        """
        d1 = (np.log(self.s0 / self.k) + (self.r + 0.5 * self.sigma ** 2) * self.t) / (self.sigma * np.sqrt(self.t))
        return norm.cdf(d1)

    def long_call_delta_CMC(self) -> float:
        """
        Calculate the delta of the European long call option, using classical Monte Carlo method.
        First, generate a sample of stock prices at maturity, then calculate the average of delta.
        """
        # TODO: implement the delta of the European long call option, using classical Monte Carlo method

    def delta(self) -> float:
        """
        :return: the delta of the European call/put option
        """
        self.check_call_put()
        self.check_long_short()
        # TODO: implement the delta of the European call/put option