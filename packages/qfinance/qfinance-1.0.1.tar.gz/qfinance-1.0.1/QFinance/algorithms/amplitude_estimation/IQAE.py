...
def inv_beta(alpha:float, x:float, y:float):
    """
    The inverse of regularized incomplete beta function. I_inverse in the paper.
    alpha: value of the incomplete beta function
    [x, y] is the interval
    """
...
class IQAE:
    ...
    In the methods of this class, we assume that every angular parameter is already scaled by 1/2*pi
    ...
    def __init__(self, problem: AEProblem):
        ...
        :param num_qubits: the number of qubits that Q acts on
        """
    ...
    def max_num_oracle_calls(self) -> int:
        """
        Return the maximum number of oracle calls, N_oracle in the reference.
        """
    ...
    def get_T(self) -> None:
        """
        Return the maximum number of rounds, T in the reference.
        """
    ...
    def calc_L_max(self) -> None:
        ...
        if self.confidence_interval_method == "CH":
            ...
            # TODO: find the L_max value numerically
            self.L_max = 1
    ...
    def run(self) -> None:
        """
        The main function of the IQAE algorithm.
        """
        ...
        # theta_{in the paper} = 2pi * theta_{in this function}
        theta_l = 0
        theta_u = 1/4
        ...
        while theta_u - theta_l > self.epsilon_ / np.pi:
            # print(f"current theta interval: [{theta_l}, {theta_u}]")
            # increment the iteration count
            iter_count += 1
            print(f"\nIteration {iter_count}...")

            if iter_count > self.max_iter_:
                raise ValueError(f"The algorithm did not converge in {iter_count} iterations, theta_u - theta_l = {theta_u - theta_l}")
    ...
    def calc_theta_from_a(self, a_min: float, a_max: float, up: bool, K: int, theta_l: float, theta_u: float):
        """
        Calculate the confidence interval [theta_l, theta_u]
        from the confidence interval [a_min, a_max] of the approximate amplitude
        """
    ...
    def find_next_K(self, k: int, theta_l: float, theta_u: float, up: bool, r: int = 2):
        """
        Implements the FindNextK subroutine in the reference.
        Find the largest k such that the interval [(4k+2)*theta_l, (4k+2)*theta_u] is fully contained in [0, pi] or [pi, 2pi].  
        In this function, we assume that every angular parameter is already scaled by 1/2*pi.
        theta_l and theta_u are in [0, 1] interval.
        theta_{in the paper} = 2pi * theta_{in this function}
        r is the ratio.
        """
        # print("find_next_K")
