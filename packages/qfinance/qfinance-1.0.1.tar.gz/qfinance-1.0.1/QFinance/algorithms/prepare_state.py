...
class SingleQubitState:
    ...
    The input state is a 2-dim vector.
    |psi> = alpha |0> + beta |1>
    The state |psi> is expected to be normalized, i.e., |alpha|^2 + |beta|^2 = 1,
    however, this class will normalize the input state if it is not.
    In the most general case, the state |psi> can be written as
    |psi> = alpha |0> + beta |1> = r * exp(i*t/2) * (exp(-i*phi/2) cos(theta/2) |0> + exp(i*phi/2) sin(theta/2) |1>)

    where r = sqrt(|alpha|^2 + |beta|^2), t = arg(alpha) + arg(beta), phi = arg(beta) - arg(alpha), theta = 2*arccos(|alpha|/r).

    The single qubit state can be rotated back to the state |0> by applying Rz and Ry gate
    Ry(-theta) Rz(-phi) |psi> = r * exp(i*t/2) |0>
    """
...
class MultiQubitState:
    """
    This class deals with the multi qubit state.
    The input state is a 2^n-dim vector.
    |psi> = sum_{i=0}^{2^n-1} alpha_i |i>
    The state |psi> is expected to be normalized, i.e., sum_{i=0}^{2^n-1} |alpha_i|^2 = 1,
    however, this class will normalize the input state if it is not.
    """
    ...
    def __init__(self, state: np.ndarray) -> None:
        ...
        :param partitiond_single_qubit_states: A list of SingleQubitState objects.
        :param thetas: A list of theta angles.
        :param phis: A list of phi angles.
        """
        ...
        if not np.isclose(np.linalg.norm(state), 1):
            print('Warning: the input state is not normalized, the algorithm will normalize it.')
        ...
        self.shots = 1000
        # self.uncompute_env = None
        self.prepare_state_env = None
    ...
    def partition_state(self, state: np.ndarray) -> tuple:
        ...
        :param state: The target state, should be a 2^n-dim ndarray.
    ...
    def prepare_state(self) -> QEnv:
        """
        prepare the state by inverse the process of uncomputing
        """
        self.uncompute_state()
        self.prepare_state_env = self.env.inverseCircuit()
        return self.prepare_state_env


    def measure_state(self) -> np.ndarray:
        """
        measure the final state self.env and return the prob vector,
        to be compared with the square@abs /@ {target state}
        """
        self.prepare_state_env.backend(BackendName.LocalBaiduSim2)
        q = self.prepare_state_env.Q
        MeasureZ(full_qreg(q), range(self.num_qubits))
        task_result = self.prepare_state_env.commit(self.shots)
    ...
    def multiplexor(self, rot_gate, angles: list, numq: int):
        """
        implement the multiplexor recursion

        :param rot_gate: The rotation gate, can be either RZ or RY.
        :param angles: An array of angles, is a list with 2^numq rotation angles.
...
class MultiQubitStateOP:
    ...
    def __init__(self, state: np.ndarray) -> None:
        ...
        :param state: The target state, should be normalized, i.e., sum_{i=0}^{2^n-1} |v_i|^2 = 1.
        :param num_qubits: The number of qubits.
        :param partitiond_single_qubit_states: A list of SingleQubitState objects.
        :param thetas: A list of theta angles.
        :param phis: A list of phi angles.
    ...
    def load_to(self, qreg_list: list[QRegStorage]):
        """
        prepare the the state by "peeling off" single qubits
        """
