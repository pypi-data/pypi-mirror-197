...
class _BaseDescr:
    ...
    def _attribute_error(self, objtype):
        raise AttributeError(
            f"'{objtype.__name__}' object has no attribute '{self.public_name}'"
        )
...
class _BBoxDescr(_GeometryDescr):
    ...
    def _set(
        self, obj: object, value: Union[object, dict, str, bytes, tuple, list]
    ) -> None:
...
class _RegexDescr(_BaseDescr):
    ...
    def __init__(
        self, regex: Union[re.Pattern, str], empty_allowed=False, **kwargs
    ) -> None:
        super().__init__(**kwargs)
...
class _ListDescr(_BaseDescr):
    ...
    def __init__(
        self, item_type=None, min_len=None, max_len=None, coerce_items=False, **kwargs
    ):
        super().__init__(**kwargs)
        ...
        if self.item_type is not None:
            if len(value) > 0:
                if not isinstance(value[0], self.item_type):
                    raise ValueError(
                        f"'{self.public_name}' must be a list of {self.item_type}"
                    )
...
class _TupleDescr(_BaseDescr):
    ...
    def _validate(self, obj: object, value: tuple) -> None:
        if not isinstance(value, (tuple, list)):
            raise ValueError(
                f"{self.public_name} must be a tuple or list, got {type(value)}"
            )
        ...
        if self.item_type is not None:
            if len(value) > 0:
                if not isinstance(value[0], self.item_type):
                    raise ValueError(
                        f"'{self.public_name}' must be a tuple of {self.item_type}"
                    )
...
class _DatetimeDescr(_BaseDescr):
    ...
    def _set(
        self, obj: object, value: Union[str, datetime.datetime, np.datetime64]
    ) -> None:
    ...
    def _validate(self, obj: object, value: str) -> None:
        ...
        if self.one_of:
            if value not in self.one_of:
                raise ValueError(
                    f'{self.public_name} must be one of {", ".join(self.one_of)}'
                )
    ...
    def _set(self, obj: object, value: object) -> None:
        ...
        if isinstance(value, str):
            ...
            if isinstance(start, str):
                ...
                raise ValueError(
                    "error setting start datetime, invalid datetime format"
                )
    ...
    def _validate(self, obj: object, value: object) -> None:
        if isinstance(value, str):
            try:
                ...
                raise ValueError(
                    "datetime as a string must be of the form <start>/<end>"
                ) from e
            ...
            for i, t in enumerate(value):
                if isinstance(t, str):
                    if t == "":
                        raise ValueError(
                            "string must be either '..' or a valid RFC3339 datetime"
                        )
...
class _TypeConstrainedDescr(_BaseDescr):
    ...
    def _validate(self, obj: object, value: object) -> None:
        if not isinstance(value, self._type):
            raise ValueError(
                f"{self.public_name} must be a {self._type}, got {type(value)}"
            )
