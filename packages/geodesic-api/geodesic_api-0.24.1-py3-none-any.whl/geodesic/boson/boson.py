...
class BosonConfig(_APIObject):
    ...
    thread_safe = _BoolDescr(
        doc="is this particular provider implementation thread safe"
    )
    pass_headers = _ListDescr(
        doc="list of headers that this provider should pass to backend"
    )
    max_page_size = _IntDescr(
        doc="the max number of records this provider can page through"
    )
    properties = _DictDescr(doc="additional provider-specific properties")
    credential = _StringDescr(
        doc="credential needed to access this dataset", deprecated=True
    )
