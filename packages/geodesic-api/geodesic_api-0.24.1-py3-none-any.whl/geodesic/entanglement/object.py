...
def delete_objects(objects_or_uids: list, project: Union[str, Project] = None):
    ...
    if all([isinstance(o, str) for o in objects_or_uids]):
        ...
        raise_on_error(
            entanglement_client.delete("objects", uids=uids, project=project)
        )
        return
...
def add_connections(
    connections: list, overwrite=False, project: Union[str, Project] = None
):
    """
...
def delete_connections(
    ...
    return raise_on_error(
        entanglement_client.delete_with_body("connections", **params)
    ).json()
...
class Object(_APIObject, metaclass=ObjectResolver):
    ...
    name = _RegexDescr(
        regex=name_re, doc="the name of this object, unique to the project"
    )
    alias = _StringDescr(
    ...
    def create(self):
        ...
        res = raise_on_error(
            self._client.post("objects", project=self.project.uid, **body)
        )
    ...
    def load(self, uid=None) -> Any:
        ...
        res = raise_on_error(
            self._client.get(f"objects/{uid}", project=self.project.uid)
        )
    ...
    def connections(
        self, version_datetime: Union[str, datetime] = None
    ) -> List["Connection"]:
        """
        ...
        if version_datetime is not None:
            # check for valid format
            if isinstance(version_datetime, str):
                params["datetime"] = datetime_to_utc(
                    isoparse(version_datetime)
                ).isoformat()
            elif isinstance(version_datetime, datetime):
                params["datetime"] = datetime_to_utc(version_datetime).isoformat()
            else:
...
class Event(Object):
    ...
    end_datetime = _DatetimeDescr(
        doc="an end time or end or some time interval for this event"
    )
...
class Predicate(_APIObject):
    ...
    domain = _RegexDescr(
        regex=qualifier_re, doc="the domain of this predicate", default="*"
    )
    category = _RegexDescr(
        regex=qualifier_re, doc="the category of this predicate", default="*"
    )
    type = _RegexDescr(
        regex=qualifier_re, doc="the type of this predicate", default="*"
    )
...
class Connection(_APIObject):
    ...
    subject = _TypeConstrainedDescr(
        (Object, dict), doc="the subject of this connection"
    )
