...
def _get_dataset(
    ...
    dataset_list = list_datasets(
        ids=[name], project=project, version_datetime=version_datetime
    )
...
class Dataset(Object):
    ...
    alias = _StringDescr(
        nested="item", doc="the alias of this object, anything you wish it to be"
    )
    ...
    stac_extensions = _ListDescr(
        nested="item", doc="list of STAC extensions this dataset uses"
    )
    ...
    version = _StringDescr(
        nested="item", doc="the version string for this dataset", default=""
    )
    ...
    def validate(self):
        res = self._client.post(
            "datasets/validate", dataset=self.item, project=self.project.uid
        )
    ...
    def create(self):
        ...
        res = raise_on_error(
            self._client.post("datasets", project=self.project.uid, **body)
        )
        ...
        if len(uids) > 1:
            raise ValueError(
                "more datasets affected than requested, something unexpected happened"
            )
    ...
    def warp(
        ...
        *,
        ...
        if content_type not in ("raw", "jpeg", "jpg", "gif", "tiff", "png"):
            raise ValueError(
                "content_type must be one of raw, jpeg, jpg, gif, tiff, png"
            )

        if resampling not in _valid_resampling:
            raise ValueError(
                f'resampling must be one of {", ".join(_valid_resampling)}'
            )
        ...
        if datetime is not None:
            req["time_range"] = [
                datetime_to_utc(parsedate(d)).isoformat() for d in datetime
            ]
    ...
    def command(self, command: str, **kwargs) -> dict:
        ...
        return raise_on_error(
            boson_client.post(
                f"command/{self.name}.{self.project.uid}/{command}", **kwargs
            )
        ).json()
    ...
    def from_arcgis_item(
        ...
        if "owner" in js:
            providers.append(
                {"name": js["owner"], "roles": ["processor"], "url": arcgis_instance}
            )
    ...
    def from_arcgis_service(
        ...
        if not url.endswith("Server"):
            raise ValueError(
                "url must end with ImageServer, FeatureServer, or MapServer"
            )
    ...
    def from_stac_collection(
        ...
        if "collections" not in url:
            raise ValueError(
                "url must be of the form {STAC_ROOT}/collections/:collectionId"
            )
        ...
        try:
            ...
            raise ValueError(
                "unable to get service metadata. (did you enter the correct url?)"
            )
        ...
        if interval[0][1] is None:
            interval[0][1] = pydatetime.datetime(2040, 1, 1).strftime(
                "%Y-%m-%dT%H:%M:%SZ"
            )
    ...
    def from_google_earth_engine(
        ...
        **kwargs,
        ...
        item_assets = {
            "image": Asset(title="image", description="image data for this dataset")
        }
    ...
    def from_csv(
        ...
        **kwargs,
        ...
        csv = dict(
            x_field=x_field, y_field=y_field, z_field=z_field, geom_field=geom_field
        )
...
class _DatasetDescr(_BaseDescr):
    ...
    def _validate(self, obj: object, value: object) -> None:
        if not isinstance(value, (Dataset, str, dict)):
            raise ValueError(
                f"'{self.public_name}' must be a Dataset or a string (name)"
            )
