...
class Feature(_APIObject):
    ...
    start_datetime = _DatetimeDescr(
        nested="properties", doc="the start timestamp of this item"
    )
    end_datetime = _DatetimeDescr(
        nested="properties", doc="the end timestamp of this item"
    )
    bbox = _BBoxDescr()
    ...
    def _validate(self, obj: object, value: object) -> None:
        ...
        if len(value) > 0:
            if not isinstance(value[0], dict):
                raise ValueError(
                    f"each value must be a dict/Feature/Item, not '{type(value[0])}'"
                )
...
class FeatureCollection(_APIObject):
    ...
    def _ipython_display_(self, **kwargs):
        return geodesic_widgets.FeatureCollectionWidget(self)._ipython_display_(
            **kwargs
        )
    ...
    def sedf(self) -> "pd.DataFrame":
        ...
        try:
            arcgis.geometry.Geometry.from_shapely(
    ...
    def from_file_geodatabase(
        ...
        """
        return FeatureCollection.from_file(
            path, layer=layer, enabled_drivers=["OpenFileGDB"]
        )
    ...
    def from_gpx(
        ...
        if layer not in valid_gpx_layers:
            raise ValueError(
                f"'{layer}' not valid, must be one of {', '.join(valid_gpx_layers)}"
            )

        return FeatureCollection.from_file(
            path, layer=layer, enabled_drivers=["GPX"], **kwargs
        )
...
class _AssetsDescr(_BaseDescr):
    ...
    def _set(self, obj: object, value: object) -> None:
        self._set_object(
            obj, {asset_name: Asset(**asset) for asset_name, asset in value.items()}
        )
...
class STACAPI:
    ...
    def collection_items(
        ...
        # Parse geospatial aspect of this query (bbox and intersects)
        params = self._query_parse_geometry(
            params, "features", bbox, None, method="GET"
        )
            ...
            if isinstance(intersects, dict):
                try:
                    ...
                    try:
                        ...
                        raise ValueError(
                            "could not determine type of intersection geometry"
                        ) from e
