...
class CQLFilterLineWidget(ipywidgets.HBox):
    ...
    def __init__(self, **kwargs):
        self._field = ipywidgets.Text(
            placeholder="<field>", layout=ipywidgets.Layout(width="90px")
        )
        ...
        self._value = ipywidgets.Text(
            placeholder="value", layout=ipywidgets.Layout(width="80px")
        )
...
class CQLFilterWidget(ipywidgets.VBox):
    def __init__(self, **kwargs):
        ...
        super().__init__(
            [self._label, self._create_row(first=True)], layout=self._layout
        )
    ...
    def _create_row(self, first=False):
        ...
        plus = ipywidgets.Button(
            description="+", layout=ipywidgets.Layout(width="30px")
        )
        plus.on_click(self._plus(len(self.children)))

        if first:
            and_or = ipywidgets.Button(
                description="clear", layout=ipywidgets.Layout(width="60px")
            )
