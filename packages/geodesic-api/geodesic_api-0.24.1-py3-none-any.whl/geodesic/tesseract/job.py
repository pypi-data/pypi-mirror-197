...
class Job(_APIObject):
    ...
    alias = _StringDescr(
        doc="a human readable name for the dataset created by this job"
    )
    description = _StringDescr(
        doc="a longer description for the dataset created by this job"
    )
    ...
    steps = _ListDescr(
        item_type=(Step, dict), doc="a list of steps to execute", coerce_items=True
    )
    hooks = _ListDescr(
    ...
    output = _TypeConstrainedDescr(
        (Bucket, dict), doc="the output, other than default storage"
    )
    ...
    def load(self, job_id: str, dry_run: bool = False) -> None:
        ...
        if "jobs" not in job_resp or len(job_resp["jobs"]) == 0:
            raise ValueError(
                f"job '{job_id}' not found in project '{self.project.uid}'"
            )
    ...
    def status(
        self, return_quark_geoms: bool = False, return_quark_status: bool = False
    ):
        ...
        res = raise_on_error(
            self._service.get(f"jobs/{self.job_id}/status", **q)
        ).json()
    ...
    def add_create_assets_step(
        ...
        * You do not need to specify the `dataset_project` unless the `dataset`'s project is ambiguous based on the
        ...
        if dataset is not None and stac_items is not None:
            raise ValueError(
                "must specify either 'dataset' or 'stac_items', but not both"
            )
        ...
        for step in self.steps:
            ...
            for input in step.inputs:
                if input.get("asset_name") is not None:
                    graph.add_node(
                        "asset:" + input.asset_name, type="asset", color="green"
                    )
    ...
    def _add_item_layer(self):
        ...
        disp.geometry = disp.geometry.buffer(
            np.sqrt(disp.geometry.area) * 0.05
        ).envelope
