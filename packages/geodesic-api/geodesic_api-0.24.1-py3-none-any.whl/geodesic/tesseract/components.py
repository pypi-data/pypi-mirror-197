...
class User(_APIObject):
    ...
    omit_empty = _BoolDescr(
        doc="don't create a space for empty bins in resulting output"
    )
    ...
    def bins(self, v: List[List[Union[datetime.datetime, str]]]):
        ...
        for d in v:
            if len(d) != 2:
                raise ValueError(
                    "bins must be a list of pairs of start/end datetime bin edges"
                )
...
class TemporalBinning(_APIObject):
    ...
    user = _TypeConstrainedDescr(
        (User, dict), doc="specify time bins defined by User time binning"
    )
...
class Webhook(_APIObject):
    ...
    credentials = _StringDescr(
        doc="name of the credential to use (geodesic.accounts.Credential)"
    )
...
class Container(_APIObject):
    ...
    repository = _StringDescr(
        doc="the docker repository to pull from", default="docker.io"
    )
    image = _StringDescr(doc="the name of the image to use")
    tag = _StringDescr(doc="the tag of the image to use")
    pull_secret_credential = _StringDescr(
        doc="name of the credential to use to pull the container"
    )
...
class BinSelection(_APIObject):
    ...
    reference_asset = _StringDescr(
        doc="which asset to reference the time bins. For step outputs"
    )
...
class StridedBinning(_APIObject):
    ...
    offset = _RegexDescr(
        bin_size_re, doc="the offset from the first date to start the bins"
    )
...
class RangeSelection(_APIObject):
    ...
    from_index = _IntDescr(
        dict_name="from", doc="starting point of bin selection in a range"
    )
...
class OutputTimeBins(_APIObject):
    ...
    nontemporal = _BoolDescr(
        doc="nontemporal; there will only be one single bin created"
    )
    user = _TypeConstrainedDescr(
        (User, dict), coerce=True, doc="user defined bin edges"
    )
...
class StepInput(_APIObject):
    ...
    type = _RegexDescr(
        r"((?:tensor)|(?:features)|(?:records))", doc="the type of input"
    )
    overlap = _IntDescr(
        doc="number of pixels of overlap to apply when tiling the inputs"
    )
    bands = _ListDescr(
        item_type=(int, float),
        coerce_items=True,
...
class StepOutput(_APIObject):
    ...
    type = _RegexDescr(
        r"((?:tensor)|(?:features)|(?:records))", doc="the type of output"
    )
...
class Step(_APIObject):
    ...
    gpu = _BoolDescr(
        doc="make the step run on a machine with GPU resources", default=False
    )
...
class GlobalProperties(_APIObject):
    ...
    chip_size = _IntDescr(
        doc="size of the chips to break work into (default=512, max=2048)"
    )
...
class AssetSpec(_APIObject):
    ...
    assets = _ListDescr(
        item_type=str,
        doc="the assets within the dataset to use. "
    ...
    pixel_dtype = _DTypeDescr(doc="output pixel dtype")
    chip_size = _IntDescr(
        doc="size of the chips to break work into (default=512, max=2000)"
    )
    compression = _StringDescr(
        one_of=["zlib", "blosc", "none"],
        doc="the compression algorithm for output data",
        ...
        if len(value) > 0:
            for asset in value:
                if not isinstance(asset, (dict, AssetSpec)):
                    raise ValueError(
                        f"each value must be a dict/AssetSpec, not '{type(asset)}'"
                    )
