...
_spatial_predicates = ["s_contains", "s_crosses", "s_disjoint", "s_equals",
                       "s_intersects", "s_overlaps", "s_touches", "s_within"]
_temporal_predicates = ["t_after", "t_before", "t_contains", "t_disjoint",
...
ops = (_binary_comparison_predicates + _and_or_predicates + _not_predicate + _comparison_predicates
       + _spatial_predicates + _temporal_predicates + _array_predicates)
...
class CQLFilter(_APIObject):
    ...
    def logical_and(*args: Tuple['CQLFilter']) -> 'CQLFilter':
        ...
        return CQLFilter(
            op="and",
            args=[CQLFilter(**x) for x in args]
        )
