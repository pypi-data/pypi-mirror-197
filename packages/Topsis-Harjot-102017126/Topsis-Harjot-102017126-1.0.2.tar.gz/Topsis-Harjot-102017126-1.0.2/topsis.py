...
#step:3 find euclidean distance and score
...
def main():
    ...
    parser.add_argument("ResultDataFile", help="Enter the name of CSV file with .csv extention",type=str)
    ...
    resultfile = args['ResultDataFile'] 
    ...
    output=pd.DataFrame(ranked_df)
    resultfile=(pd.concat([d1,output],axis=1))

    pd.DataFrame(resultfile).to_csv('102017126-result.csv')
    print('Done')
