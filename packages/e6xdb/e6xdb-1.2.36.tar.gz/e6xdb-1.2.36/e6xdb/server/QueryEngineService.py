...
class Iface(object):
    ...
    def setOrUnsetCaches(self, sessionId, bIsCacheSet):
        """
        Parameters:
         - sessionId
         - bIsCacheSet
...
class Client(Iface):
    ...
    def setOrUnsetCaches(self, sessionId, bIsCacheSet):
        ...
        """
        self.send_setOrUnsetCaches(sessionId, bIsCacheSet)
        self.recv_setOrUnsetCaches()

    def send_setOrUnsetCaches(self, sessionId, bIsCacheSet):
        self._oprot.writeMessageBegin('setOrUnsetCaches', TMessageType.CALL, self._seqid)
        args = setOrUnsetCaches_args()
        args.sessionId = sessionId
        args.bIsCacheSet = bIsCacheSet
        args.write(self._oprot)
        self._oprot.writeMessageEnd()
        self._oprot.trans.flush()

    def recv_setOrUnsetCaches(self):
        iprot = self._iprot
        (fname, mtype, rseqid) = iprot.readMessageBegin()
        if mtype == TMessageType.EXCEPTION:
            x = TApplicationException()
            ...
            iprot.readMessageEnd()
            raise x
        result = setOrUnsetCaches_result()
        result.read(iprot)
        iprot.readMessageEnd()
        if result.error2 is not None:
            raise result.error2
...
class Processor(Iface, TProcessor):
    def __init__(self, handler):
        ...
        self._processMap["setOrUnsetCaches"] = Processor.process_setOrUnsetCaches
    ...
    def process_setOrUnsetCaches(self, seqid, iprot, oprot):
        args = setOrUnsetCaches_args()
        args.read(iprot)
        iprot.readMessageEnd()
        result = setOrUnsetCaches_result()
        try:
            self._handler.setOrUnsetCaches(args.sessionId, args.bIsCacheSet)
            msg_type = TMessageType.REPLY
        except TTransport.TTransportException:
            raise
        except AccessDeniedException as error2:
            ...
            msg_type = TMessageType.EXCEPTION
            result = TApplicationException(TApplicationException.INTERNAL_ERROR, 'Internal error')
        oprot.writeMessageBegin("setOrUnsetCaches", msg_type, seqid)
        result.write(oprot)
        oprot.writeMessageEnd()
        oprot.trans.flush()
...
class setOrUnsetCaches_args(object):
    """
    Attributes:
     - sessionId
     - bIsCacheSet
    ...
    def __init__(self, sessionId=None, bIsCacheSet=None,):
        self.sessionId = sessionId
        self.bIsCacheSet = bIsCacheSet
    ...
    def read(self, iprot):
        ...
        while True:
            ...
            if fid == 1:
                ...
                if ftype == TType.BOOL:
                    self.bIsCacheSet = iprot.readBool()
                else:
                    iprot.skip(ftype)
            else:
                iprot.skip(ftype)
    ...
    def write(self, oprot):
        if oprot._fast_encode is not None and self.thrift_spec is not None:
            oprot.trans.write(oprot._fast_encode(self, [self.__class__, self.thrift_spec]))
            return
        oprot.writeStructBegin('setOrUnsetCaches_args')
        if self.sessionId is not None:
            oprot.writeFieldBegin('sessionId', TType.STRING, 1)
            oprot.writeString(self.sessionId.encode('utf-8') if sys.version_info[0] == 2 else self.sessionId)
            oprot.writeFieldEnd()
        if self.bIsCacheSet is not None:
            oprot.writeFieldBegin('bIsCacheSet', TType.BOOL, 2)
            oprot.writeBool(self.bIsCacheSet)
            oprot.writeFieldEnd()
        oprot.writeFieldStop()
        oprot.writeStructEnd()
    ...
    def __ne__(self, other):
        return not (self == other)
all_structs.append(setOrUnsetCaches_args)
setOrUnsetCaches_args.thrift_spec = (
    None,  # 0
    (1, TType.STRING, 'sessionId', 'UTF8', None, ),  # 1
    (2, TType.BOOL, 'bIsCacheSet', None, None, ),  # 2
)


class setOrUnsetCaches_result(object):
    """
    Attributes:
     - error2
    ...
    def write(self, oprot):
        if oprot._fast_encode is not None and self.thrift_spec is not None:
            oprot.trans.write(oprot._fast_encode(self, [self.__class__, self.thrift_spec]))
            return
        oprot.writeStructBegin('setOrUnsetCaches_result')
        if self.error2 is not None:
    ...
    def __ne__(self, other):
        return not (self == other)
all_structs.append(setOrUnsetCaches_result)
setOrUnsetCaches_result.thrift_spec = (
    None,  # 0
