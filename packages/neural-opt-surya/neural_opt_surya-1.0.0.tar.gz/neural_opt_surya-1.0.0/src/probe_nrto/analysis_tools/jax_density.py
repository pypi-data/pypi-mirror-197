...
Can help form the Eigenvalue Density Spectrum from the
 outputs of Stochastic Lanczos Quadrature"""
...
def eigv_to_density(eig_vals, all_weights=None, grids=None,
  ...
  density_each_draw = np.zeros((num_draws, grid_len))#density of each iteration
  ...
  norm_fact = np.sum(density) * (grids[1] - grids[0])#sum * bin width
  density = density / norm_fact #S: for normalized plots -- yes--area under graph= 1
  return density, grids


def tridiag_to_eigv(tridiag_list):
  """Preprocess the tridiagonal matrices for density estimation.
  Makes the spectral decompsoition of T = U*L*U^T
  Then the nodes are the eigen values = L_i,i
  And the weights are : 
  ...
  for i in range(num_draws):
    nodes, evecs = np.linalg.eigh(tridiag_list[i])#Eigen vectors in columns
    index = np.argsort(nodes)
    nodes = nodes[index]
    evecs = evecs[:, index]
    eig_vals[i, :] = nodes
    all_weights[i, :] = evecs[0] ** 2#First component of eigen vector for each eigen value             
...
def tridiag_to_density(tridiag_list, sigma_squared=1e-5, grid_len=10000):
  ...
  """
  #eigen_vals:Contains the eigen values of each run as rows
      #it is a matrix of (k,order) 
  eig_vals, all_weights = tridiag_to_eigv(tridiag_list)
  #all_weights:similar shape as eigen_vals
          #But each row is the square of the minimum eigen vector
