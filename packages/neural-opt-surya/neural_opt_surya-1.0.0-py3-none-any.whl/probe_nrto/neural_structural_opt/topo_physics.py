...
def f_eta(x, beta, eta):
    x_phy = ( np.tanh(beta*eta) + np.tanh(beta*(x-eta)))/(np.tanh(beta*eta) + np.tanh(beta*(1-eta)))
    y = x_phy.mean() - x.mean()
    return y
...
def physical_density(x, args, volume_contraint=False, cone_filter=True, 
                     den_proj =False, beta = 1):#, etavals = []):
  """
  Main differnce :  Cone_filter application, Now works for elements sizes other than 1
  args['filter_width']: Physical domain size over which the filter has to act 
  args['filter_width'] / elem_size :  Number of elements to be included for cone_filtering
  """
  ...
  if volume_contraint:
      ...
      x = x * args['mask']
  if cone_filter: # works only with a square element
      assert args['width']/args['nelx'] == args['height']/ args['nely']
      elem_size = args['width']/args['nelx']
      x = autograd_lib.cone_filter(x, 2,  args['mask'])
      #args['filter_width'] / elem_size, args['mask'])
  if den_proj:
      # Heavyside projector
      if not isinstance(x, np.ndarray): # When its an arraybox for autograd
          f = lambda eta : f_eta(x._value, beta, eta)
      else:
          f = lambda eta : f_eta(x, beta, eta) # For rendering designs
      eta = my_bisection(f, 0, 1, 1e-3)  
      #etavals.append(eta)
...
def mean_density(x, args, volume_contraint=False, cone_filter=True, den_proj =False,
                 beta=1):
  return (np.mean(physical_density(x, args, volume_contraint, cone_filter, den_proj, beta))
          / np.mean(args['mask']))


def get_stiffness_matrix(args):
  """
  New stiffness matrix that can accept different sizes for the element edges
  Obtained from symbolic calculations
  """
  ...
  k = np.array([12*H**2-6*W**2*nu+6*W**2,  4.5*H*W*(1+nu),  -12*H**2-3*W**2*nu+3*W**2,
                36*H*W*(-0.125+0.375*nu),  -6*H**2+3*W**2*nu-3*W**2,  -4.5*H*W*(1+nu),
                6*H**2+6*W**2*nu-6*W**2,  36*H*W*(0.125-0.375*nu)])
  
  return e/(36*H*W*(1-nu**2))*np.array([[k[0], k[1], k[2], k[3], k[4], k[5], k[6], k[7]],
...
def compliance_ele(x_phys, u, ke, *, penal = 1, e_min=1e-9, e_0=1):
  # Calculates the compliance of elements
  ...
  ce = np.einsum('ijk,ijk->jk', u_selected, ke_u).T*young_modulus(x_phys, e_0, e_min, p=penal)
...
def calculate_forces(x_phys, args):
  ...
  # distribute the desnity to the different nodes [forces act of nodes]
  density = 0
  for pad_left in [0, 1]:
    for pad_up in [0, 1]:
  ...
  gravitional_force = -args['g'] * density[..., np.newaxis] * np.array([0, 1]) * elem_vol
  # Forces along x = 0 [array[0]] correspnds to this
  return applied_force + gravitional_force.ravel()


def objective(x, ke, args, volume_contraint=False, cone_filter=True, p=3.0,
  ...
  x_phys = physical_density(x, args, volume_contraint=volume_contraint,
                            cone_filter=cone_filter, den_proj=den_proj, beta=beta)
