...
class DebSpdx:
    ...
    # #####CHANGE##### #
    def init_treated_list(self, start, set_command_dict):
        self.treated_list.clear()
        self.start = start
        self.set_command_dict = set_command_dict
        self.set_command_dict[start] = {}
    ...
    def add_relationship(self, d_list: list[str]) -> list[str]:
        ...
        for ori_list in dori_list:
            for or_list in ori_list:
                ...
                # #####CHANGE##### #
                if or_list[0] in pv_dict:
                    self.c_counter += 1  # バージョン条件確認
                    if self.check_version(or_list[1:], ["=", pv_dict[or_list[0]]]):
            ...
            # 上の階層のパッケージと相互依存になっているとき か すでに上の階層のパッケージの別の枝で調査済みのとき
                ...
                # #####CHANGE##### #
                self.set_command_dict[self.start][real_dp_name] = self.c_counter
