...
def main():
    ...
    try:                                                                       input_par['cpnest-branch'] = Config.get("Sampler settings",'cpnest-branch')
    except(KeyError, configparser.NoOptionError, configparser.NoSectionError): input_par['cpnest-branch'] = 'master'
    if(input_par['cpnest-branch']=='master'):
    ...
    elif(input_par['cpnest-branch']=='massively-parallel'):
        sampler_params={'verbose'                     : 2,
                        'nlive'                       : 1024,
                        'maxmcmc'                     : 1024,
                        'nnest'                       : 1,
        ...
        raise ValueError('Unsupported cpnest branch passed under the option `cpnest-branch` of the `Sampler settings` section.')
    ...
    if (input_par['run-type']=='full'):
        ...
        if(input_par['cpnest-branch']=='master'):
            ...
            try   : import ray
            except: raise Exception('The `massively-parallel` version of `cpnest` requires the ray package. Please install it through `pip install ray`.')
