...
def extract_info(image: ee.Image, county: ee.Geometry, n_rois: int = 5) -> list:
    ...
    try:
        ...
        rois = ee.FeatureCollection(points.map(lambda feature: ee.Feature(feature.geometry().buffer(5000))))
        ...
        for roi in rois.getInfo()['features'] :
            ...
            geemap.ee_export_image(image, filename, region=ee.Feature(roi, {}).geometry(), scale=scale, file_per_band=False)
        ...
        # Compute the mean value of each spectral index for all ROIs
        segment_mean = np.mean(roi_means, axis=0)

        # Return a list of the county geometry, date, and spectral indices
        return segment_mean

    except Exception as e:
        raise Exception(f"Error in extract_info: {str(e)}")
