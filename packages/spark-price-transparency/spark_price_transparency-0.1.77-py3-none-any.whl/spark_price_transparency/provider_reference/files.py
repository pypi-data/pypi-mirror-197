...
class Provider_reference(Pt_files):
    ...
    def run_import(self):
        ...
        for r in loc_files:
            self.run_import_url(r.url, r.tgt_file_location)
    ...
    def run_import_df(self, url_df: DataFrame):
        ...
        loc_files = self._spark.table('pt_stage.in-pr_loc').filter(col('mth') == lit(self.mth)).alias('loc') \
            .join(url_df.alias('url'), col('url.url') == col('loc.location'), 'inner') \
            .select('url', 'tgt_file_name').collect()
        for r in loc_files:
            self.run_import_url(r.url, r.tgt_file_location)
