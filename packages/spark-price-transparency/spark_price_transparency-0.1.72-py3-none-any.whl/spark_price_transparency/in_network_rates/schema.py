...
from ..pt_functions import col, F, cols_arrangement, col_sk_coverage, col_sk_provider, col_sk_pr_loc
...
class In_network_rates(Pt_schema):
    ...
    def run_analytic(self):
        file_name_df = self.inr_header.select('file_name').distinct() \
            .join(self.in_coverage.select(col('inr_file_name').alias('file_name')).distinct(),
                  "file_name", "left_anti")
        self.run_analytic_df(file_name_df)

    def run_analytic_df(self, file_name_df: DataFrame):
        file_name_df.cache()
        header = F.broadcast(self.inr_header
        ...
        network = self.inr_network.repartition(800) \
            .join(file_name_df, "file_name", "left_semi") \
        ...
        in_coverage = header.select(col('mth'),
                                    col('file_name').alias('inr_file_name'),
            ...
            .join(network.select(col('file_name').alias('inr_file_name'),
                                 col('sk_coverage'),
                                 col('arrangement.*')),
                  'inr_file_name', 'left').distinct()

        rate = network.join(header, 'file_name', 'left') \
            .withColumn('negotiated_rate', F.explode('negotiated_rates')) \
            .select('file_name', 'reporting_entity_name', 'network.mth', 'sk_coverage', 'negotiated_rate.*').alias('network') \
