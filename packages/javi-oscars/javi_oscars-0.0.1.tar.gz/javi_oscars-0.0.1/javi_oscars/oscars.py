...
class BaseModel(metaclass=MetaModel):
    ...
    def _init_(self, **row_data):
        for field_name, value in row_data.items():
            setattr(self, field_name, value)
 
    def _repr_(self):
        attrs_format = ", ".join([f'{field}={value}' for field, value in self._dict_.items()])
        return f"<{self._class.name_}: ({attrs_format})>"
