...
class API:
    ...
    def _put_extra_headers(self, kwargs):
        ...
        auth_values = []
        if self._auth_token or "login" in kwargs:
            if extra_headers is None:
                extra_headers = {}
                kwargs["headers"] = extra_headers
            if self._auth_token:
                auth_values.append(f"Bearer {self._auth_token}")
            if "login" in kwargs:
                ...
                auth_values.append(auth.encode())
            if auth_values:
                extra_headers["Authorization"] = ",".join(auth_values)
