...
def get_structure_list(df: str = "aimd.h5")-> List[Structure]:
    ...
    if df.endswith(".h5"):
        ...
        Nstep, elements, positions, lattices = _read_h5(df)
        strs = []
        for i in range(Nstep):
            strs.append(
                Structure(lattices[i], elements, positions[i], coords_are_cartesian=True)
            )
...
class RDF:
    ...
    def get_one_rdf(
        ...
        counts = self._dist_to_counts(distances) # 分析距离，统计目标原子数
