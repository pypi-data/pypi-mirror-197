...
class LVMTemplateForm(NetBoxModelForm):
    ...
    def save(self, *args, **kwargs):
        ...
        #pv = PhysicalVolume.objects.create(device=pv_linux_device, vg=volume_group)
        #pv.save()
        #print(f"Physical Volume ID: {pv.id}")

        # Add linux_device for lvm
        pv_type_pk = ContentType.objects.get(app_label='netbox_storage', model='volumegroup').pk
        linux_device_mapper = LinuxDevice.objects.create(device='/dev/mapper/vg_system-lv_home',
                                                         type='LVM',
                                                         size=pv_linux_device.size,
                                                         object_id=volume_group.pk,
                                                         content_type_id=pv_type_pk)
...
class DriveTemplateForm(NetBoxModelForm):
    ...
    def save(self, *args, **kwargs):
        ...
        linux_device = LinuxDevice.objects.create(device=drive.device_name(),
                                                  type='disk',
