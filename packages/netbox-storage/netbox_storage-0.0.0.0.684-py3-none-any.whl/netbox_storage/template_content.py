...
class TemplateVolumeConfig(PluginTemplateExtension):
    ...
    def full_width_page(self):
        ...
        for drive_id in drives:
            ...
            if LinuxDevice.objects.filter(object=drive_id['drive']).count() == 0:
                drive = Drive.objects.get(pk=drive_id['drive'])
                print(f"Lonely Drive: {drive}")
                lonely_drive.append(drive)
            else:
                linux_devices.append(list[LinuxDevice.objects.filter(object=drive_id['drive'])])
                print(LinuxDevice.objects.filter(object=drive_id['drive']).first().drives)
