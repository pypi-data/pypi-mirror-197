...
def run(input_dir='',             
    ...
    if bounding_box == 'face' or bounding_box == 'yolov5s' or bounding_box == 'rotated':
        ...
        if bounding_box in ['face', 'yolov5s','rotated']:
            ...
            if max_index > out_len:
                exploded_df = pd.DataFrame({'index':range(0,max_index)})
                out_df = exploded_df.merge(out_df, how='left', on='index')
                out_df['filename'].filena('missing_image')
