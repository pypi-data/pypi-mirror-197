...
@publish_invocation
def cmd_pod_load(url_or_name=_A,url=_A,remote=_A,strategy=MergeStrategyPretty.from_merge_strategy(DEFAULT_MERGE_STRATEGY)):
	...
	if B or G:C=E.import_pod(source=B or A,merge_strategy=D);F(C);return C
	if not licensing.is_logged_in():console.print(_E);return
	C=E.pull_state(pod_name=A,remote_config=RemoteConfig(remote_url=remote),merge_strategy=D)
