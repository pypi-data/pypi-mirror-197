...
class CloudPodsVersionManager(CloudPodsManager):
	...
	def push(A,comment=_A,services=_A):
		B=comment;A.pods_api.set_pod_context(A.pod_name);A._add_state_to_cloud_pods_store(extract_assets=_B,services=services);C:bool;D=A.pods_api.get_head().version_number
		if A.remote.is_registered(A.pod_name):
			E=A.remote.get_max_version(pod_name=A.pod_name);C=D<E
			if C:A.pull()
			A.pods_api.push(comment=B);A.remote.push_pod(A.pod_name,version=D)
		else:F=A.pods_api.push(comment=B);LOG.debug("Created new version %s for pod '%s'",F.version_number,A.pod_name)
		return PodInfo()
	...
	def inject(A,version=-1,merge_strategy=_A):
		...
		except Exception as G:LOG.debug('An exception occurred while trying to load Cloud Pod %s: "%s"',A.pod_name,G);return _C
		finally:
			if E:rm_rf(B)
...
class PodConfigManager(metaclass=PodConfigManagerMeta):
	...
	def pod_config(D,pod_name):
		...
		return B[0]
def get_pods_manager(pods_name,remote_config=_A):return CloudPodsVersionManager(pod_name=pods_name,remote_config=remote_config)
...
def set_public(pod_name,public):
	A=pod_name;B=get_pods_manager(A);B.pods_api.set_pod_context(A);C=CloudPodsRemotePlatform(B.pods_api.config_context)
	if not C.is_registered(A):return _C
	D=CloudPodsRemotePlatform.create_platform_url(A);E=get_auth_headers();F=safe_requests.patch(D,headers=E,data=json.dumps({'is_public':public}));return F.ok
