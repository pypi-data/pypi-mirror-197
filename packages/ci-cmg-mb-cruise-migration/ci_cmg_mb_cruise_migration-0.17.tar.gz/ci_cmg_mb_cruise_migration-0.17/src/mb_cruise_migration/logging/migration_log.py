...
class MigrationLog(LoggerBuilder):
    ...
    @staticmethod
    def log_no_raw_processed_product(transfers):
        if transfers:
            survey = transfers[0].mb_survey_crate.mb_survey.survey_name
            MigrationLog.logger.warning(f"No raw, processed, or product dataset found for survey {survey}. Files will be migrated without a related platform.")
        else:
            MigrationLog.logger.warning(f"Attempted to log no raw, product, or processed dataset when no datasets existed at all.")
