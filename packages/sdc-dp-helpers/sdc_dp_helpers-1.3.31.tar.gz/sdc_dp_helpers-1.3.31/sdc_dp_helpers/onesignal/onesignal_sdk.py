...
class CreateBulkDownload(APICallHandler):
    ...
    @request_handler(wait=0.5)
    @retry_handler(ConnectionError, total_tries=5, initial_wait=1, backoff_factor=2)
    @retry_handler(
    ...
    def api_call(self, session: requests.Session, url: str):
        ...
        try:
            ...
            if response.status_code == 404:
                raise NoDownloadYetError() from exc
            raise exc
...
class ViewNotificationsHandler(OneSignalHandler):
    ...
    def fetch_data(self):
        ...
        while more_records:
            more_records, results = api_call_handler.api_call(
