...
from getnet.domain.customers import Customer
from getnet.utils import HandlerRequest, handler_request_exception
...
class Client(object):
    ...
    def _handler_request(self):
        return HandlerRequest(self, LOGGER)
    ...
    def get(self, path, **kwargs) -> dict:
        ...
        Args:
            ...
            **kwargs:
        """
        with HandlerRequest(self, LOGGER):
    ...
    def customer_service(self) -> CustomersService:
        """Return a instance of Customer Service
        Returns:
            CustomersService
        """
    ...
    def payment_customer(self, customer) -> payments.Customer:
        """Return a instance of Payment Customer

        Args:
            customer (Customer): Payment User Data
        Returns:
            payments.Customer
        """
    ...
    def payment_credit_service(self) -> payments.credit.Service:
        """Return a instance of Payment Credit Card Service

        Returns:
            payments.credit.Service
        """
    ...
    def create_credit_transaction(self, amount, delayed, pre_authorization, save_card_data, transaction_type, number_installments, order, customer, card, device, shipping_address, currency="BRL") -> payments.credit.Service:
        credit = {
            "delayed": delayed,
        ...
        return self.payment_credit_service().create(amount, currency, order, credit, customer, device, shipping_address)

    def cancel_credit_transaction(self, payment_id) -> CreditCancelPaymentResponse:
        """Cancel a Credit Card Transaction

        Args:
            payment_id (int): Payment ID
        Return:
            CreditCancelPaymentResponse.credit_cancel.message
        """
        return self.payment_credit_service().cancel(payment_id)

    def capture_credit_transaction(self, payment_id: str, amount: str) -> CreditCapturePaymentResponse:
        """Capture a Credit Card Transaction

        Args:
            payment_id (int): Payment ID
            amount (str): Amount to capture
        Return:
            CreditCapturePaymentResponse.credit_confirm.message
        """
