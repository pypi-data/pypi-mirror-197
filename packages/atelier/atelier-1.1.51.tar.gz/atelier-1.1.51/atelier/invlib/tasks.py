...
def install(ctx, list=False, batch=False, after=None, start=None):
    ...
    output = run_in_demo_projects(ctx, cmd, bare=True, after=after, start=start)
...
@task(name='prep', help=dict(
    after="Run demo_prep_command only in the demo projects *after* the named one.",
    start="Run demo_prep_command only in the demo projects *starting at* the named one."))
