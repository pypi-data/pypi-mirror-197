    ...
    def samples(self, integration: dict) -> list[Sample]:
        ...
        try:
            ...
            for stream in streams:
                ...
                if stream.metadata.metadata.get('table-key-properties'):
                    table_keys = stream.metadata.metadata.get('table-key-properties')
                    if table_keys[0] == '_key':
                        reserved_keys.remove('_key')
                LOGGER.info(f"Debug 2:")
                for reserved_key in reserved_keys:
                    if s_schema.properties.get(reserved_key):
                        new_key = "_" + reserved_key
                        while True:
                            if s_schema.properties.get(new_key):
                                new_key = "_" + new_key
                            else:
                                break
                            LOGGER.info(f"Debug 3:")
                        s_schema.properties[new_key] = s_schema.properties.pop(reserved_key)
                        LOGGER.info(f"Debug 4:")
                LOGGER.info(f"S_Schema properties is : {s_schema.properties}")
        ...
        try:
            ...
            for stream in streams:
                ...
                if md_map.get((), {}).get('table-key-properties'):
                    table_keys = md_map.get((), {}).get('table-key-properties')
                    if table_keys[0] == '_key':
                        reserved_keys.remove('_key')
                for reserved_key in reserved_keys:
                    if s_schema.properties.get(reserved_key):
