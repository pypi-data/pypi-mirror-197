...
rsvidx.lsh_custom_hash_get.argtypes = [c_void_p, c_size_t, c_ulong, c_void_p]
rsvidx.lsh_custom_hash_get.restype = c_ulong
...
class Similarity:
	...
	def get_custom_hash(self, hash: int, limit: int = 100):
		buffer_size = (id_size + (sizeof(c_float) * self._dimensions)) * limit
		buffer = create_string_buffer(buffer_size)
		result_size = rsvidx.lsh_custom_hash_get(
			self._idx,
			hash,
			limit,
			buffer
