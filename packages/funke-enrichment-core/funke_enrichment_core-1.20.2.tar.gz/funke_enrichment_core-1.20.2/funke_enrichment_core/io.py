...
from funke_enrichment_core.helper import publish_data_to_pubsub_topic, warning, err, make_dict_json_serializable
...
class WriteToBigQuery():
    ...
    def _truncate_table(self, table_id):
        #TODO: Make a proper truncate table query job
        query = "TRUNCATE TABLE `{}`".format(table_id)
        return query
        ...
        for field in self.table_schema:
            ...
            if data_field is not None:
                ...
                try:
                    if caster == dict:
                        if field.mode == 'REPEATED':
                            data[field.name] = [self._type_casting(entry) for entry in data_field]
                        else:
                            data[field.name] = self._type_casting(data_field)
                    elif caster is not None:
                        if field.mode == 'REPEATED':
                            data[field.name] = [caster(entry) for entry in data_field]
                        else:
    ...
    async def process(self, inp_data):
        ...
        try:
            errors = self.bq_client.insert_rows_json(table=self.table,
                                                     json_rows=[make_dict_json_serializable(inp_data)],
