...
@click.pass_context
@coro
async def test_run(ctx: click.Context, file: Tuple[str, ...], verbose: bool, only_fail: bool):
...
@ click.pass_context
@ coro
async def test_init(ctx: click.Context, files: Tuple[str, ...], force: bool):
    if len(files) == 0:
...
@ click.pass_context
@ coro
async def test_parse(ctx: click.Context, files: Tuple[str, ...]):
    for f in files:
        click.echo(f"\nFile: {f}")
