...
class Booth(SiteVisitor):
    NAME = 'Booth'
    URL_REGEX: re.Pattern = re.compile(HTTP_REGEX + r'(?P<id>\w+).booth.pm', re.IGNORECASE)
    ...
    def visit(self, url, context: Context, id: str):
        res = requests.get(f'https://{id}.booth.pm')
        ...
        description = desc_element is not None and desc_element.find(attrs={'v-html': 'formattedDescription'}) or None
        ...
        context.create_result(
            ...
            profile_picture=avater_element.attrs['style'].split('url(')[1].split(')')[0] if avater_element is not None else None
        )
