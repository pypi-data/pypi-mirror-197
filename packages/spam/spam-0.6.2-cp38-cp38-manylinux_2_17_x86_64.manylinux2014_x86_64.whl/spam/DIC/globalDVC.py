...
from spam.DIC.DICToolkit import computeDICglobalMatrix, computeDICglobalVector
...
def _makeRegularisationMatrix(regularisation, points, connectivity, Mc):
    ...
    try:
        ...
        raise KeyError(f"Missing parameter in BULK: {e}")
    # 1.2 check cast not mandatory BULK
    BULK_FEW = BULK.get("few_times", 3.0)  # we still need to figure out what it means
    print(f"DEBUG: bulk young = {BULK_YOUNG}")
    print(f"DEBUG: bulk poisson = {BULK_POISSON}")
    print(f"DEBUG: bulk ksi = {BULK_KSI}")
    print(f"DEBUG: bulk few = {BULK_FEW}")
    ...
    print(f'DEBUG: dirichlet -> {"yes" if DIRICHLET else "no"}')
    ...
    print(f"DEBUG: number of dirichlet surfaces -> {n_dirichlet}")
    DSd = []
    DSd_A = []
    DSd_B = []
    DSd_nn = []
    ...
    if MESH_TYPE == "cuboid":
        ...
        print(f"DEBUG: compute surfaces based on min/max mesh coordinates {surfaces_positions}")
        ...
        def _dirpos(key, surfaces_positions):
            ...
            print(f"DEBUG: dirichlet BC {key} -> {direction}, {position}")
        ...
        print(f"DEBUG: dirichlet BC bcd_lookup = {bcd_lookup}")

        # loop over the points
        # PUT SOME NUMBA HERE
        for A, point in enumerate(points):
            # print(f"DEBUG: point {A} {point} ({A/points.shape[0]*100:.2f}%)")
            ...
            for direction, position in enumerate(surfaces_positions):
                ...
                if abs(point[direction] - position) < 0.00001:
                    # print(f"DEBUG: Point number {A}: {point} in surface N{direction} = {position}")
    ...
    # print(f"DEBUG: k = {k} |k| = {numpy.linalg.norm(k)} 1/|k| = {1.0 / numpy.linalg.norm(k)}")
    ...
    print(f"DEBUG: Ec = {Ec}, Em = {Em}, Es = {Es}")
    ...
    for _, ksi in DIRICHLET_SURFACES:
        if ksi == 0:  # compute value based on k_mag
            ksi = 1.0 / k_mag
            print(f"DEBUG: use 1/k_mag for ksi for surface {_}: {ksi}")
    ...
    print(
        f"DEBUG: omega_t = {omega_t}, \n\
                    omega_c = {omega_c} ({omega_c/omega_t}), \n\
                    omega_m = {omega_m} ({omega_m/omega_t}), \n\
                    omega_s = {omega_s} ({[_/omega_t for _ in omega_s]})"
    )
    print(f"DEBUG: weight_m = {weight_m}, weight_s = {weight_s}")
...
def globalCorrelation(
    ...
    if nThreads is None:
        ...
        print(f"spam.DIC.globalCorrelation(): C++ parallelisation on all threads ({nThreads})")
    else:
        print(f"spam.DIC.globalCorrelation(): C++ parallelisation on {nThreads} threads (user input)")

    if verbose:
        print(f"spam.DIC.globalCorrelation(): convergenceCriterion = {convergenceCriterion}")
        print(f"spam.DIC.globalCorrelation(): maxIterations = {maxIterations}")
    ...
    if verbose:
        ...
        print(f"\t\tmin coords: {minCoord}")
        print(f"\t\tmax coords: {maxCoord}")
    ...
    if initialDisplacements is None:
        ...
        print(f"DEBUG: run labelTetrahedra: {time.perf_counter()-tic} seconds.")
    ...
    while dxNorm > convergenceCriterion and i < maxIterations:
        ...
        if verbose:
            tic = time.perf_counter()
            print("\t\t\trun computeDICglobalVector: ", end="")
        ...
        if verbose:
            print(f"{time.perf_counter()-tic} seconds.")
        ...
        if medianFilterEachIteration:
            ...
            # if verbose:
            #     print("\tspam.DIC.globalCorrelation(): Starting median filter of displacements...", end='')
        ...
        if verbose:
            tic = time.perf_counter()
            print("\t\t\trun applyMeshTransformation: ", end="")
        im1Def = spam.DIC.applyMeshTransformation(
            im1,
            points,
            connectivity,
        ...
        if verbose:
            ...
            print(f"\ti={i:03d}, displacements {strMin}, {strMed}, {strMax}, dx={dxNorm:.2f}")
