...
def readStructuredVTK(f, fieldAsked=None):
    ...
    pl = {
        ...
        if line.split()[0] in ["CELLS", "POINTS", "CELL_DATA", "POINT_DATA"]
    }
    ...
    for key in keys_valid.items():
        ...
        if key[1][1] == "SCALARS":
            fields.update(
                {key[0]: numpy.array([float(r) for r in content[l_start:l_end]])}
            )
        else:
            fields.update(
                {
                    key[0]: numpy.array(
                        [
                            [float(c) for c in reversed(r.split())]
                            for r in content[l_start:l_end]
                        ]
                    )
                }
            )
...
def writeStructuredVTK(
    ...
    if len(cellData) + len(pointData) == 0:
        print(
            "spam.helpers.writeStructuredVTK() Empty files. Not writing {}".format(
                fileName
            )
        )
        return 0
    ...
    if len(cellData):
        ...
        for k, v in cellData.items():
            if set(dimensionsCell) != set(v.shape[:3]):
                print(
                    "spam.helpers.writeStructuredVTK() Inconsistent cell field sizes {} != {}".format(
                        dimensionsCell, v.shape[:3]
                    )
                )
                return 0
    ...
    if len(pointData):
        ...
        for k, v in pointData.items():
            if set(dimensionsPoint) != set(v.shape[:3]):
                print(
                    "spam.helpers.writeStructuredVTK() Inconsistent point field sizes {} != {}".format(
                        dimensionsPoint, v.shape[:3]
                    )
                )
                return 0
    ...
    if len(pointData):
        for k, v in pointData.items():
            if dimension != v.shape[0]:
                print(
                    "spam.helpers.writeGlyphsVTK() Inconsistent point field sizes {} != {}".format(
                        dimension, v.shape[0]
                    )
                )
                return 0
        ...
        print(
            "spam.helpers.writeGlyphsVTK() Empty files. Not writing {}".format(fileName)
        )
        return
...
def TIFFtoVTK(fileName, voxelSize=1.0):
    ...
    # create coordinates
    points = spam.mesh.createLexicoCoordinates(
        [voxelSize * n for n in nCells], [n + 1 for n in nCells]
    )
