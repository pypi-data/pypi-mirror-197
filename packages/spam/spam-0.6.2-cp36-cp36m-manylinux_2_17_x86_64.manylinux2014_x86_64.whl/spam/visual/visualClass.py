...
class ereg(QWidget):
    ...
    def showImages(self):
        ...
        if self.comparisonMode == 0:  # check the comparison mode and then display the computed comparison
            if self.axis == "z":
                self.comparison.setImage(qimage2ndarray.array2qimage(numpy.subtract(self.images[0][self.slice[0][0], :, :], self.images[1][self.slice[1][0], :, :]), normalize=True))
            elif self.axis == "y":
                self.comparison.setImage(qimage2ndarray.array2qimage(numpy.subtract(self.images[0][:, self.slice[0][1], :], self.images[1][:, self.slice[1][1], :]), normalize=True))
            elif self.axis == "x":
                self.comparison.setImage(qimage2ndarray.array2qimage(numpy.subtract(self.images[0][:, :, self.slice[0][2]], self.images[1][:, :, self.slice[1][2]]), normalize=True))
        elif self.comparisonMode == 1:
            if self.axis == "z":
                self.comparison.setImage(qimage2ndarray.array2qimage(numpy.absolute(numpy.subtract(self.images[0][self.slice[0][0], :, :], self.images[1][self.slice[1][0], :, :])), normalize=True))
            elif self.axis == "y":
                self.comparison.setImage(qimage2ndarray.array2qimage(numpy.absolute(numpy.subtract(self.images[0][:, self.slice[0][1], :], self.images[1][:, self.slice[1][1], :])), normalize=True))
            elif self.axis == "x":
                self.comparison.setImage(qimage2ndarray.array2qimage(numpy.absolute(numpy.subtract(self.images[0][:, :, self.slice[0][2]], self.images[1][:, :, self.slice[1][2]])), normalize=True))
        elif self.comparisonMode == 2:
            if self.axis == "z":
                if self.changeColorCheck.isChecked():
                    self.comparison.setImage(
                        qimage2ndarray.array2qimage(
                            cGM.checkerBoard(self.images[0][self.slice[0][0], :, :] + self.images[1].max() * (1 / 6), self.images[1][self.slice[1][0], :, :], self.nbCB, False, False), normalize=True
                        )
                    )
                else:
                    self.comparison.setImage(
                        qimage2ndarray.array2qimage(cGM.checkerBoard(self.images[0][self.slice[0][0], :, :], self.images[1][self.slice[1][0], :, :], self.nbCB, False, False), normalize=True)
                    )
            elif self.axis == "y":
                if self.changeColorCheck.isChecked():
                    self.comparison.setImage(
                        qimage2ndarray.array2qimage(
                            cGM.checkerBoard(self.images[0][:, self.slice[0][1], :] + self.images[1].max() * (1 / 6), self.images[1][:, self.slice[1][1], :], self.nbCB, False, False), normalize=True
                        )
                    )
                else:
                    self.comparison.setImage(
                        qimage2ndarray.array2qimage(cGM.checkerBoard(self.images[0][:, self.slice[0][1], :], self.images[1][:, self.slice[1][1], :], self.nbCB, False, False), normalize=True)
                    )
            elif self.axis == "x":
                if self.changeColorCheck.isChecked():
                    self.comparison.setImage(
                        qimage2ndarray.array2qimage(
                            cGM.checkerBoard(self.images[0][:, :, self.slice[0][2]] + self.images[1].max() * (1 / 6), self.images[1][:, :, self.slice[1][2]], self.nbCB, False, False), normalize=True
                        )
                    )
                else:
                    self.comparison.setImage(
                        qimage2ndarray.array2qimage(cGM.checkerBoard(self.images[0][:, :, self.slice[0][2]], self.images[1][:, :, self.slice[1][2]], self.nbCB, False, False), normalize=True)
                    )
    ...
    def modifyPhi(self, mod, value):
        ...
        # create a transformation dictionary
        transformation = {"t": [self.tArray[0], self.tArray[1], self.tArray[2]], "r": [self.rArray[0], self.rArray[1], self.rArray[2]], "z": [self.zArray[0], self.zArray[1], self.zArray[2]]}
...
class QtCropWidget(QWidget):
    ...
    def saveTSVCrop(self):
        if self.nameEntry.text() != "":
            ...
            output = numpy.array([[self.z[0]], [self.z[1]], [self.y[0]], [self.y[1]], [self.x[0]], [self.x[1]], [self.binning]])
            numpy.savetxt(fileName, output.T, fmt="%.7f", delimiter="\t", newline="\n", comments="", header=TSVheader)
...
class JHist(QWidget):
    def __init__(self, images, phi, crop, fontColor, names, imUpdate):
        ...
        self.greyLimitsOrig = [[self.images[0].min(), self.images[0].max()], [self.images[1].min(), self.images[1].max()]]
        self.greyLimitsCrop = [[self.images[0].min(), self.images[0].max()], [self.images[1].min(), self.images[1].max()]]
    ...
    def reFit(self, index):
        ...
        while j < len(self.gaussianParameters) and not find:
            if self.gaussianParameters[j][0] == self.maxima[index][0] and self.gaussianParameters[j][1] == self.maxima[index][1]:
                ...
                find = True
            j += 1
        tmpGaussianP = [self.maxima[index][0], self.maxima[index][1], self.maxima[index][2], a, b, c]
        self.gaussianParameters.append(tmpGaussianP)
...
class FinalStep(QWidget):
    def __init__(self, images, crop, phaseDiagram, phi, GP, bins, greyLimitsCrop, binning, deformedImageName, names):
        ...
        self.margin = 5
        margin = (slice(5, self.imagesCrop[0].shape[0] - 5), slice(5, self.imagesCrop[0].shape[1] - 5), slice(5, self.imagesCrop[0].shape[2] - 5))
        ...
        DICToolkit.computeGMresidualAndPhase(self.imagesCrop[0][margin], self.im2CropDef[margin], self.phaseDiagram, self.gaussianParameters, residualField, phaseField)
        ...
        self.viewerCheckerBoard.setImage(qimage2ndarray.array2qimage(cGM.checkerBoard(self.imagesCrop[0][margin][middleSlice, :, :], self.im2CropDef[margin][middleSlice, :, :]), normalize=True))
    ...
    def starCorrelation(self):
        ...
        margin = (slice(m, im1.shape[0] - m), slice(m, im1.shape[1] - m), slice(m, im1.shape[2] - m))
        ...
        while (iterations <= maxIterations and deltaPhiNorm > deltaPhiMin) or iterations <= 1:
            ...
            # Compute operators
            DICToolkit.computeDICoperatorsGM(
            ...
            DICToolkit.computeGMresidualAndPhase(im1[margin].astype("<f4"), im2def[margin].astype("<f4"), phaseDiagram.astype("<u1"), gaussianParameters.astype("<f8"), residualField, phaseField)
            ...
            self.viewerCheckerBoard.setImage(qimage2ndarray.array2qimage(cGM.checkerBoard(im1[margin][middleSlice, :, :], im2def[margin][middleSlice, :, :]), normalize=True))
            #
