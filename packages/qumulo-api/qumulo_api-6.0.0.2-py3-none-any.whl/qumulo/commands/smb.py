...
def get_id_from_args(rest_client: RestClient, args: argparse.Namespace, smb_mod: Any) -> int:
    share_id = args.id
    if args.name:
        # Share names are no longer unique with multitenancy so both are required when querying by
        # name if multitenancy is enabled
        # Since shares will either all have a tenant_id or all not have a tenant_id, we can check
        # for multitenancy if any share has that field populated. If we have no shares, it doesn't
        # matter if we can't disambiguate share names since there are no shares.
        res = smb_mod.smb_list_shares(rest_client.conninfo, rest_client.credentials)
        shares = res.data['entries']
        if len(shares) != 0 and shares[0]['tenant_id'] is not None:
            if args.name and not args.tenant_id:
                raise ValueError('Must specify --tenant-id with --name if multitenancy is enabled')
        # Find share id by name and tenant id
        for share in shares:
            if share['share_name'] == args.name and share['tenant_id'] == args.tenant_id:
                share_id = share['id']

    if share_id is None:
        raise ValueError(f"SMB Share with name {args.name} doesn't exist")
