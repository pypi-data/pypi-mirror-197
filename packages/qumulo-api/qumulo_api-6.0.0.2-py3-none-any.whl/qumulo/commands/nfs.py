...
def get_id_from_args(rest_client: RestClient, args: argparse.Namespace) -> int:
    ...
    if args.export_path:
        # Export paths are no longer unique with multitenancy so both are required when querying by
        # name if multitenancy is enabled
        # Since exports will either all have a tenant_id or all not have a tenant_id, we can check
        # for multitenancy if any share has that field populated. If we have no exports, it doesn't
        # matter if we can't disambiguate exports paths since there are no exports.
        exports = nfs.nfs_list_exports(rest_client.conninfo, rest_client.credentials).data[
            'entries'
        ]
        if len(exports) != 0 and exports[0]['tenant_id'] is not None:
            if args.export_path and not args.tenant_id:
                raise ValueError(
                    'Must specify --tenant-id with --export-path if multitenancy is enabled'
                )
        # Find export id by export path and tenant id
        for export in exports:
            if export['export_path'] == args.export_path and export['tenant_id'] == args.tenant_id:
                export_id = export['id']

    if export_id is None:
        raise ValueError(f"NFS Export with path {args.export_path} doesn't exist")
