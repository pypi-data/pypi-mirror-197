...
def pitches_weighed_average(pitch_segment):
    ...
    for _i in range(len(pitches)):
        result_list.append(pitches[_i] * duration_list[_i] / sum(duration_list))
...
def tendentiousness(pitch_segment):
    ...
    flag_dict = {}
    for _i in range(1, len(duration_list)):
        for i1 in range(0, _i + 1):
            group1.append(duration_list[i1])
        for i2 in range(_i + 1, len(duration_list)):
            group2.append(duration_list[i2])
        flag_dict[_i] = abs(np.sum(group1) - np.sum(group2))
    # 暂时先把最小差值赋给split
    ...
    t2 = []
    for _i in range(split):
        t1.append(duration_list[_i])
    for _i in range(split, len(duration_list)):
        t2.append(duration_list[_i])
...
class PitchSegment:
    ...
    def __init__(self, segment, __style=None):
        ...
        self.total_duration = np.sum(self.duration_set)
