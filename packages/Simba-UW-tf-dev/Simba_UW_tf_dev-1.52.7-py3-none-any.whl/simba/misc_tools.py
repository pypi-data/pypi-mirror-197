...
def smooth_data_savitzky_golay(config: configparser.ConfigParser,
    ...
    if not video_file_path:
        print(f'SIMBA ERROR: Import video for {filename} to perform Savitzky-Golay smoothing')
        raise FileNotFoundError(f'SIMBA ERROR: Import video for {filename} to perform Savitzky-Golay smoothing')
...
def copy_single_video_to_project(simba_ini_path: str,
    ...
    if os.path.isfile(destination):
        print('SIMBA ERROR: {} already exist in SimBA project. To import, delete this video file before importing the new video file with the same name.'.format(
                file_name))
...
def copy_multiple_videos_to_project(config_path: str,
    ...
    if len(video_path_lst) == 0:
        print('SIMBA ERROR: No videos found in {} directory in file-type {}'.format(source, file_type))
        raise ValueError()
