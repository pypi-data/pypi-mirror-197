...
from simba.utils.lookups import get_body_part_configurations, get_bp_config_codes
...
class ProjectCreatorPopUp(PopUpMixin):
    def __init__(self):
        ...
        parent_tab.add(self.create_project_tab, text=f'{"[ Create project config ]": ^20s}')
        parent_tab.add(self.import_videos_tab, text=f'{"[ Import videos ]": ^20s}')
        parent_tab.add(self.import_data_tab, text=f'{"[ Import tracking data ]": ^20s}')
        parent_tab.add(self.extract_frms_tab, text=f'{"[ Extract frames ]": ^20s}')
