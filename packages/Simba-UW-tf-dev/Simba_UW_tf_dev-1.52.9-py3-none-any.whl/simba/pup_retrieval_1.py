...
def pup_retrieval_1(cofigini, prob_pup, prob_mother, dist_start_crit, carry_frames_seconds, smooth_factor, corenest_name, nest_name, dam_name, pup_name, smooth_function, max_time, carry_classifier_name, approach_classifier_name, dig_classifier_name):
    ...
    dist_pup_cornest_col = f'{corenest_name} {pup_name} {"distance"}'
    dist_mother_cornest_col = f'{corenest_name} {dam_name} {"distance"}'
    pup_in_corenest_col = f'{corenest_name} {pup_name} {"in zone"}'
    pup_in_nest_col = f'{nest_name} {pup_name} {"in zone"}'
    ...
    for counter, video_file in enumerate(features_files):
        ...
        currVideoSettings, currPixPerMM, video_fps = read_video_info(vid_info_df=video_info_df, video_name=video_base_name_no_file_ending)
        if max_time != 'none':
