...
from simba.utils.errors import (BORISTimeOverlapError,
                                BORISEventCountError)
import pandas as pd
        ...
        for file_cnt, file_path in enumerate(self.feature_file_paths):
            ...
            for clf in self.clf_names:
                ...
                if len(clf_annotations_start) != len(clf_annotations_stop):
                    raise BORISEventCountError(video_name=self.video_name, clf_name=self.clf, start_event_cnt=len(clf_annotations_start), stop_event_cnt=len(clf_annotations_stop))
                ...
                if len(results) > 0:
                    raise BORISTimeOverlapError(video_name=self.video_name, clf_name=self.clf)
