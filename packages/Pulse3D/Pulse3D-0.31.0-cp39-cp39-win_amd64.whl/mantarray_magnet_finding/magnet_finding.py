...
def get_positions(
    ...
    **initial_guess_config: Dict[str, Union[int, float]],
    ...
    for incr in range(0, 10): # change x position guess incrementally, uniformly for all wells until the algorithm converges properly
        b_meas = data[:, 0]
        prev_guess = prev_guess + np.append(np.ones(NUM_ACTIVE_WELLS) * -GUESS_INCR,
                                            np.zeros((NUM_PARAMS - 1) * NUM_ACTIVE_WELLS))  # increment previous guess  # increment previous guess
