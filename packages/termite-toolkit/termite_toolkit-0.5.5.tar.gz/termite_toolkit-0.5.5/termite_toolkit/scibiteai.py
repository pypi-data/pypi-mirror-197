...
class SciBiteAIClient():
	...
	def ner_from_sent(self, models, sent, res_format='scibite',hits_only=False):
		...
		if type(models) == list:
			models = ','.join(models)
		hits_only = 'true' if hits_only else 'false'
		req = f'/ner/predict_sentences?models={models}&res_format={res_format}&hits_only={hits_only}'
		data = {'sentences': sent}
		r = requests.post('http://' + self.scibiteai_url + req, json=data, auth=self.scibiteai_auth)
		return r.json()
...
def pairwise_markup(docjsonx, pairwise_types_a, pairwise_types_b, normalisation='id', 
					wrap=False, wrapChars=('{!', '!}'), substitute=True, replacementDict=None):
	'''
	...
	for combo in combos:
		...
		output[combo] =markup(docjsonx, vocabs=pairwise_types_a+pairwise_types_b, normalisation=normalisation, 
					wrap=wrap, wrapChars=wrapChars, substitute=substitute, 
...
def pairwise_text_markup(text, pairwise_types_a, pairwise_types_b, termiteAddr='http://localhost:9090/termite',
				normalisation='id', wrap=False, wrapChars=('{!', '!}'), substitute=True, replacementDict=None,
				termite_http_user=None, termite_http_pass=None, include_json=False):
	'''
	...
	if include_json:
		return pairwise_markup(docjsonx, pairwise_types_a=pairwise_types_a, pairwise_types_b=pairwise_types_b, 
								normalisation=normalisation, wrap=wrap, wrapChars=wrapChars, substitute=substitute, 
								replacementDict=replacementDict), docjsonx
	else:
		return pairwise_markup(docjsonx, pairwise_types_a=pairwise_types_a, pairwise_types_b=pairwise_types_b, 
								normalisation=normalisation, wrap=wrap, wrapChars=wrapChars, substitute=substitute, 
