...
@LastEditTime: 2023-03-14 12:05:09
...
class TaskBaseModel(FrameBaseModel):
    ...
    def get_client_task_list_v2(self, app_id, act_id, module_id, user_id, task_types="", daily_repeat_browse=False):
        ...
        for task_info_dict in task_info_list:
            ...
            result["task_type"] = task_info_dict["task_type"]
            result["title"] = task_info_dict["task_name"]
            if result["task_type"] == TaskType.weekly_sign.value.value:
                result["reward_value"] = config_json["day_list"] if config_json.__contains__("day_list") else []
            elif result["task_type"] == TaskType.cumulative_sign.value.value:
                day_list = config_json["day_list"] if config_json.__contains__("day_list") else []
                if len(day_list) == 1:
                    result["reward_value"] = config_json["day_list"][0][str(max(config_json["day_list"]))]
                elif len(day_list) > 1:
                    result["reward_value"] = config_json["day_list"] if config_json.__contains__("day_list") else []
                else:
            ...
            #邀请助力
                ...
                invite_help_list = invite_help_model.get_cache_dict_list("id_md5=%s and user_id=%s", "", order_by="id desc", field="invited_avatar", params=[help_id_md5, user_id], dependency_key=DependencyKey.invite_help_list(id_md5, user_id))
    ...
    def process_invite_help(self,app_id,act_id,module_id,user_id,login_token,from_user_id,handler_name,continue_request_expire=5,check_act_info_release=True,check_act_module_release=True):
        """
        ...
        """
        invoke_result_data = InvokeResultData()
        acquire_lock_name = f"process_invite_help:{act_id}_{module_id}_{user_id}"
        identifier = ""
        request_queue_name = ""
        task_type = TaskType.invite_new_user.value
        now_day = TimeExHelper.get_now_day_int()
        ...
        try:
            ...
            invoke_result_data = self.business_process_executing(app_id,act_id,module_id,user_id,login_token,handler_name,False,False,continue_request_expire,acquire_lock_name,check_act_info_release=check_act_info_release,check_act_module_release=check_act_module_release)
            if invoke_result_data.success == True:
                act_info_dict = invoke_result_data.data["act_info_dict"]
                act_module_dict = invoke_result_data.data["act_module_dict"]
                user_info_dict = invoke_result_data.data["user_info_dict"]
                ...
                if invite_help_dict:
                    ...
                    if task_invoke_result_data.success == True:
                        ...
                        if is_invite_limit == True:
                            ...
                            task_count.modify_day = now_day
                            task_count_model.add_update_entity(task_count,"complete_count=%s,now_count=%s,modify_date=%s,modify_day=%s",params=[task_count.complete_count,task_count.now_count,task_count.modify_date,now_day])
                            invite_help_model.delete_dependency_key(DependencyKey.invite_help_list(help_id_md5,from_user_id))
                    else:
                        invoke_result_data.success = False
                        invoke_result_data.error_code = task_invoke_result_data.error_code
                        invoke_result_data.error_message = task_invoke_result_data.error_message
    ...
    def process_secret_code(self,app_id,act_id,module_id,user_id,login_token,handler_name,secret_code,check_new_user=False,check_user_nick=False,continue_request_expire=5):
        """
