...
from geraldo3.utils import get_attr_value, calculate_size, memoize
from geraldo3.widgets import Widget, Label, SystemField
from geraldo3.graphics import Graphic, RoundRect, Rect, Line, Circle, Arc, \
    Ellipse, Image
from geraldo3.barcodes import BarCode
from geraldo3.base import GeraldoObject, ManyElements, ReportBand
from geraldo3.cache import CACHE_BY_QUERYSET, CACHE_BY_RENDER, CACHE_DISABLED, \
    make_hash_key, get_cache_backend
from geraldo3.charts import BaseChart
from geraldo3.exceptions import AbortEvent
...
class ReportGenerator(GeraldoObject):
    ...
    def make_band_rect(self, band, top_position, left_position):
        ...
        band_rect = {
            ...
            'top': top_position,
            'right': left_position + self.calculate_size(band.width),
            # self.report.page_size[0] - self.report.margin_right,
        ...
        if isinstance(element, Widget):
            ...
            if isinstance(widget, SystemField):
                ...
                if widget.truncate_overflow:
                    ...
                    widget.left = band_rect['left'] + self.calculate_size(widget.left)
                    widget.top = self.calculate_top(temp_top, self.calculate_size(widget.top),
                                                    self.calculate_size(para.height))
    ...
    def render_band(self, band: ReportBand, top_position=None, left_position=None,
        ...
        self._rendered_pages[-1].width = self.calculate_size(self.report.page_size[0]) - \
            self.calculate_size(self.report.margin_left) - \
            self.calculate_size(self.report.margin_right)
            ...
            while self._current_object_index < len(objects):
                ...
                if done != False:
                    if self.get_available_height() < self.calculate_size(d_band.height):
                        ...
                        d_width = self.calculate_size(d_band.width) + self.calculate_size(
                            getattr(d_band, 'margin_left', 0))
    ...
    def render_subreports(self):
        ...
        for subreport in self.report.subreports:
            ...
            if not subreport.BandDetail or not subreport.visible:
                continue
            ...
            for num, obj in enumerate(self._current_queryset):
                ...
                # Forces new page if there is no available space
                force_new_page(subreport.BandDetail.height)

                # Renders the detail band
                if subreport.BandDetail.visible:
                    self.render_band(subreport.BandDetail, current_object=obj)
