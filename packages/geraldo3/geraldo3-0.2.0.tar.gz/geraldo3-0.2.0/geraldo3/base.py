...
class GeraldoObject(object):
    ...
    def find_by_name(self, name, many=False):
        """Find child by informed name (and raises an exception if it doesn't
        find).
    ...
    def find_by_type(self, typ):
        """Find child by informed type (and raises an exception if it doesn't).
...
class BaseReport(GeraldoObject):
    ...
    # Data source driver
    queryset = None
    print_if_empty = False  # if true when a queryset is empty, the report will still be generated
    ...
    def transform_classes_to_objects(self):
        """Finds all band classes in the report and instantiate them. This
        is important to have a safety to separate inherited reports."""
    ...
    @staticmethod
    def get_variable_value(name, system_fields):
...
class ReportMetaclass(type):
    ...
    def __new__(mcs, name, bases, attrs):
        ...
        new_class = super(ReportMetaclass, mcs).__new__(mcs, name, bases, attrs)
...
class Report(BaseReport):
    ...
    def generate_under_process_by(self, generator_class, *args, **kwargs):
        ...
        a Process and save memory consumming, with better use of multicore servers.
        
        This just will work well if you are generating in a destination file or
        file-like object (i.e. an HttpResponse on Django).
        
        It doesn't return anything because Process doesn't."""
    ...
    def get_page_rect(self):
        ...
        if not self._page_rect:
            client_width = calculate_size(self.page_size[0]) - calculate_size(self.margin_left) - calculate_size(
                self.margin_right)
            client_height = calculate_size(self.page_size[1]) - calculate_size(self.margin_top) - calculate_size(
                self.margin_bottom)
...
class ReportBand(GeraldoObject):
    ...
    def transform_classes_to_objects(self):
        """Finds all child band classes in this class and instantiate them. This
        is important to have a safety on the separated inherited reports"""
