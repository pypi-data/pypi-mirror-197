...
def test_mege_strategy(tmp_datasets_dir):
    ...
    for i in range(len(ds)):
        assert np.all(cpp_ds.tensors[0][i] == ds.x[i].numpy())
        assert np.all(cpp_ds.tensors[1][i] == ds.y[i].numpy())
...
def test_rename_merge(tmp_datasets_dir):
    ...
    with local_ds as ds:
        ...
        assert np.all(cpp_ds.tensors[1][0] == [2, 3, 4])
        ...
        assert np.all(cpp_ds.tensors[1][0] == [2, 3, 4])
        assert np.all(cpp_ds.tensors[1][1] == [1, 0, 0])
        ...
        assert np.all(cpp_ds.tensors[1][0] == [2, 3, 4])
        assert np.all(cpp_ds.tensors[2][0] == [0, 0, 1])
        assert np.all(cpp_ds.tensors[2][1] == [2, 3, 4])
        assert np.all(cpp_ds.tensors[2][2] == [1, 0, 0])