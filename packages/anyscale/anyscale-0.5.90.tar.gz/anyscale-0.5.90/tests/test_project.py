...
def test_get_and_validate_project_id():
    ...
    api_client = Mock()
    anyscale_api_client.get_default_project = Mock(
        return_value=Mock(result=Mock(id="mock_project_id4"))
    )
    ...
    mock_get_cloud_id_and_name.assert_called_once_with(
        api_client=api_client, cloud_id=None, cloud_name="mock_parent_cloud_name",
    )

    # Test using default project without correct parameters
    anyscale_api_client = Mock()
    api_client = Mock()
    api_client.check_is_feature_flag_on_api_v2_userinfo_check_is_feature_flag_on_get = Mock(
        return_value=Mock(result=Mock(is_on=True))
