...
def _validate_parse_function(
    ...
    if get_origin(return_annotation) in (UnionType, Union):
        ...
        real_return_arg = next((a for a in return_args if a is not type(None)), None)
        if len(return_args) != 2 or type(None) not in return_args or real_return_arg is None:
            raise TypeError(
                f"Parse function '{parse_fqn}' return annotation must be of length one (optional is supported)"
            )
        output_optional = True
    else:
        real_return_arg = return_annotation
    if not issubclass(real_return_arg, BaseModel):
    ...
    if real_return_arg not in (param.annotation for param in stream_fn_inputs.values()):
        raise TypeError(
    ...
    inputs = [annotation_parser.parse_annotation(p) for p in sig.parameters.keys()]
    if len(inputs) != 1:
        raise TypeError(f"Parse function '{parse_fqn}' must have one input argument")
    real_input = inputs[0]
    # Could possibly check to see if output is part of input, not yet implemented

    if not issubclass(real_input, BaseModel):
        raise TypeError(f"Parse function '{parse_fqn}' return annotation must be of type pydantic.BaseModel")
    return ParseInfo(
        fn=parse_fn, input_type=real_input, output_type=real_return_arg, output_is_optional=output_optional
    )
