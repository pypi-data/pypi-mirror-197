...
def get_optimizer(optimizer_name, learning_rate):
    ...
    if type == 'constant':
        ...
        try:
            learning_rate_func, kwargs = get_learning_rate(learning_rate_name=lr_schedule_name, **learning_rate)
...
def get_learning_rate(learning_rate_name, **kwargs):
    if learning_rate_name == 'constant':
        ...
        return tensorflow.keras.optimizers.schedules.ExponentialDecay(lr, decay_steps, decay_rate, staircase), kwargs
    elif learning_rate_name == 'PiecewiseConstantDecay':
        boundaries = kwargs.pop('boundaries')
        values = kwargs.pop('values')
        return tensorflow.keras.optimizers.schedules.PiecewiseConstantDecay(boundaries, values), kwargs
        ...
        return tensorflow.keras.optimizers.schedules.PolynomialDecay(lr, decay_steps, end_learning_rate, power, cycle), kwargs
    elif learning_rate_name == 'InverseTimeDecay':
        lr = kwargs.pop('initial_learning_rate')
        decay_steps = kwargs.pop('decay_steps')
        decay_rate = kwargs.pop('decay_rate')
        staircase = kwargs.pop('staircase', False)
        return tensorflow.keras.optimizers.schedules.InverseTimeDecay(lr, decay_steps, decay_rate, staircase), kwargs
    elif learning_rate_name == 'CosineDecay':
        lr = kwargs.get('initial_learning_rate')
        decay_steps = kwargs.pop('decay_steps')
        alpha = kwargs.pop('alpha', 0.0)
        return tensorflow.keras.optimizers.schedules.CosineDecay(lr, decay_steps, alpha), kwargs
        ...
        return tensorflow.keras.optimizers.schedules.CosineDecayRestarts(lr, first_decay_steps, t_mul, m_mul, alpha), kwargs
    elif learning_rate_name == 'LinearCosineDecay' or learning_rate_name == 'NoisyLinearCosineDecay':
        raise ValueError(f'{learning_rate_name} not supported in latest tensorflow version')
    else:
