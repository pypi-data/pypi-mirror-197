    ...
    def optimizer(self, optimizer: str):
        ...
        try:
            ...
            if self.__learningRateSet:
                optlrrateflag, error = check_parameters.get_optimizer(optimizer, self.__learningRate.copy())
    ...
    def learningRate(self, learningRate: dict):
        ...
        if TYPE in learningRate.keys():
            if self.__optimizerSet:
                optlrrateflag, error = check_parameters.get_optimizer(self.__optimizer, learningRate.copy())
    ...
    def reducelrCallback(
        ...
        try:
            ...
            if type(factor) == float and type(patience) == int and type(min_delta) == float:
                ...
                error_msg = "Invalid datatype for arguments given for reducelrCallback\n"
                self.__print_error(error_msg)
        except:
            error_msg = f"Please provide supported monitor values: {f}\n"
            self.__print_error(error_msg)
