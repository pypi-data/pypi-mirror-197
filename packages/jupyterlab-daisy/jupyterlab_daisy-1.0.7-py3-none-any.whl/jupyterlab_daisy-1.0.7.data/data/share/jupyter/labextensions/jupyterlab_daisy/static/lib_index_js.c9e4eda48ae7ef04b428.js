    ...
    populateListRelated(source_asset_name, target_asset_names, list) {
        ...
        (0,_handler__WEBPACK_IMPORTED_MODULE_5__.requestAPI)(`get-related?source_asset_id=${source_asset_name}&target_asset_ids=${target_asset_names.join(',')}`)
            ...
            json['RelatedTables'].forEach((entry) => {
                ...
                bla.setAttribute('title', `Click '+' for details...`);
                const button = document.createElement('button');
                button.className = 'my-button';
                button.textContent = '+';
                const text = document.createElement('p');
                text.textContent = entry.links[entry.links.length - 1].split('/')[0];
                ...
                tr.innerHTML = `
                        <td>${splitLinkSource[splitLinkSource.length - 2]}/${splitLinkSource[splitLinkSource.length - 1]}</td>
                        <td class='alnright'>${splitLinkTarget[splitLinkTarget.length - 2]}/${splitLinkTarget[splitLinkTarget.length - 1]}</td>
                      `;
                ...
                if (activeCell !== null) {
                    ...
                    const addField = function (node) {
                        ...
                        for (let i = 0; i < additionalFields.childElementCount; i++) {
                            additionalFields.children[i].children[0].placeholder = `Asset ${i}`;
                        }
                    };
...
//# sourceMappingURL=lib_index_js.c9e4eda48ae7ef04b428.js.map