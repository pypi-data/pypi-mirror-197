...
from .model import InferenceSpec, Model
from .model import ResourceConfig as InferenceResourceConfig
...
class Estimator(object):
    ...
    Example::
        ...
        est = Estimator(
            ...
            output_path="oss://{YOUR_BUCKET_NAME}/pai/training-job/output_path",
        )
    ...
    def __init__(
        ...
        Args:
            ...
            entry_point (str): The entry point file used by the job, which can be a
                `.py` or `.sh` file. The TrainingJob will be run using the command
                `python {entry_point} --hp-name-1 hp-value-1 --hp-name-2 hp-value-2`.
            source_code (str, optional): The local source code directory used in the
            ...
            hyperparameters (dict, optional): A dictionary that represents the
                ...
                stored in the `/ml/input/config/hyper-parameters.json` as a JSON
            ...
            base_job_name (str, optional): The base name used to generate the training
                job name.
            checkpoint_path (str, optional): An OSS URI that stores the checkpoint of the
            ...
            output_path (str, optional): An OSS URI to store the outputs of the training
                ...
                A completed training container directory structure example::
                    ...
                    |   `-- config
                    |       |-- hyper-parameters.json	// Hyperparameters in JSON
    ...
    def _gen_job_display_name(self, job_name=None):
        ...
        return "{}_{}".format(self.base_job_name or "trainingjob", ts)
    ...
    def _put_source_if_not_exists(self, src, object_key):
        oss_bucket = self.session.oss_bucket  # type: oss2.Bucket
        try:
    ...
    def _upload_source_files(self, job_name):
        ...
        if self.source_code:
            ...
            # if source_code is not provided, use entry_point as source files.
        ...
        if not isinstance(source_file, six.string_types):
            raise ValueError(
                f"Unsupported source_files type, expected string or GitConfig, but "
                f"given {type(self.source_code)}"
            )

        if source_file.startswith("oss://"):
            # source file in OSS bucket is not required uploading.
            self.__uploaded_source_files = self.source_code
        else:
            # compress the source files to a Tar Gz file and upload to OSS bucket.
            upload_data_path = self.session.get_oss_storage_path(
                "training_src", to_plain_text(job_name)
            )
            self.__uploaded_source_files = upload_data(
    ...
    def _build_command(self):
        ...
        return [
            # TrainingService download the source code passing by `CodeDir` and untar
    ...
    def _build_input_data_configs(self, inputs: Dict[str, Any] = None):
        ...
        for name, item in inputs.items():
            if isinstance(item, six.string_types):
                ...
                res.append({"Name": name, "DatasetId": item.dataset_id})
            else:
                raise ValueError(
                    "The Estimator support OSS URI, NAS URI, or Dataset as input data,"
    ...
    def _build_output_data_configs(self, job_name: str):
        ...
        # Use checkpoint_path from user or construct a checkpoint path using
        # default output path.
        checkpoints_path = self.checkpoint_path or os.path.join(
            job_base_output_path,
    ...
    def _build_code_input(self, job_name: str) -> Optional[Dict[str, Any]]:
        ...
        if oss_uri_obj.endpoint:
            code_dir["LocationValue"]["Endpoint"] = oss_uri_obj.endpoint
    ...
    def _generate_job_base_output_path(self, job_name):
        ...
        # replace no alpha-number in job name.
        name = to_plain_text(job_name)

        if self.output_path:
            return os.path.join(self.output_path, f"{name}_{random_str(6)}")
        else:
            job_output_path = self.session.get_oss_storage_path(
                "training-job", f"{name}_{random_str(6)}"
            )
    ...
    def fit(self, inputs: Dict[str, Any] = None, wait=True):
        ...
        Args:
            inputs (Dict[str, Any]): A dictionary representing the input data for the
                ...
                data can be an OSS URI or a :class:`pai.dataset.Dataset` object and will
                be mounted to the `/ml/input/data/{channel_name}` directory in the
                training container.
            wait (bool): If block utils the training job is completed: succeeded,
                failed, or stopped.
        """
        inputs = inputs or dict()
    ...
    def model_data(self):
        ...
        if not self._latest_job:
            raise RuntimeError(
                "No TrainingJob for the estimator, not found output model data."
            )
        return self._latest_job.output_path(
    ...
    def deploy(
        ...
        resource_config: Optional[
            Union[InferenceResourceConfig, Dict[str, int]]
        ] = None,
        ...
        **kwargs,
        ...
        Args:
            ...
            resource_config (InferenceResourceConfig): Resource config for each
                prediction service.
            serializer (SerializerBase):
            wait (bool): If wait until the service is ready.
...
class _LocalTrainingJob(object):
    """A class represent a local training job running with docker container."""
    ...
    def run(self):
        ...
        shutil.copytree(self.estimator.source_code, user_code_dir)
    ...
    def prepare_input_config(self, input_config_path):
        ...
        with open(os.path.join(input_config_path, "training-job.json"), "w") as f:
            ...
            # if launch job using code in /ml/usercode.
            training_job_spec = {
                "codeDir": {
                    "locationType": "oss",
                    "locationValue": {
        ...
        for name, input_data in self.inputs.items():
            ...
            if input_data.startswith("oss://"):
                ...
                oss_bucket = self.session.get_oss_bucket(
                    oss_uri_obj.bucket_name, endpoint=oss_uri_obj.endpoint
                )
                makedirs(local_channel_path)
                download_data(
...
class _TrainingJob(EntityBaseMixin):
    ...
    def output_path(self, channel_name="model"):
        ...
        raise RuntimeError(
            f"Not found output model path: model_channel_name={channel_name}"
        )
