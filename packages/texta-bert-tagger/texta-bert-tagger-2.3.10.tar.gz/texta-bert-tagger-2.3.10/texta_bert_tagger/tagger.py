...
class BertTagger:
    ...
    def tag_text(self, text: str, highest_only: bool = True) -> Dict[str, Union[str, float]]:
        ...
        #print(self.config.label_reverse_index)
        predicted_label = self.config.label_reverse_index[prediction]
        
        all_preds = [
            {"prediction": self.config.label_reverse_index[i], "probability": probabilities[0][i]}
        ...
        if highest_only:
            out = {"prediction": predicted_label, "probability": probability}
        else:
            out = all_preds
