...
class Screen:
    ...
    def drawLine(self, x0, y0, x1, y1, bg=None, fg=None, char=" "):
        if x0 == x1 and y0 == y1:
            self.drawPixel(x0, x1, bg=bg, fg=fg, char=char)
            return
        dx = (x1 - x0)
        dy = (y1 - y0)
        ...
        while not (round(x) == x1 and round(y) == y1):
            self.drawPixel(round(x), round(y), bg=bg, fg=fg, char=char)
            x += xinc
            y += yinc
        self.drawPixel(round(x1), round(y1), bg=bg, fg=fg, char=char)

    def drawLineCustom(self, start_x, start_y, end_x, end_y, bg=None, fg=None, char=" "):
        curr_x = start_x
        curr_y = start_y
    ...
    def drawText(self, x, y, text, bg=None, fg=None):
        for idx, char in enumerate(str(text)):
            self.drawPixel(x+idx, y, bg=bg, fg=fg, char=char)


    def blit(self):
        result = ""
        previous_pixel = Pixel(Color(0), Color(255), " ")
        for x_p, i in enumerate(self.matrix):
            for y_p, j in enumerate(i):
                if self.doColor:
                    result += j.getPixelTransparent(previous_pixel, self.prev_matrix[x_p][y_p])
                    previous_pixel = j
                else:
                    result += j.char
            result += "\n"
...
class Pixel:
    ...
    def getPixelTransparent(self, prev_pixel, bg_pixel):
        r_bg = self.bg
        if r_bg == "": r_bg = bg_pixel.bg
        r_fg = self.fg
        if r_fg == "": r_fg = bg_pixel.fg

        if r_bg == prev_pixel.bg: r_bg = ""
        if r_fg == prev_pixel.fg: r_fg = ""
...
class Color:
    def __init__(self, *args):
        ...
        if len(args) == 1 or len(args) == 2:
            if str(args[0]) == "transparent" or args[0] == None:
                self.r = self.g = self.b = 0
                self.t = True
            else:
