...
class HyperV:
    ...
    def path_exists(self, vm_name=''):
        vm_patch=f"{self.storage_root}\{self.env_prefix}\{self.env_prefix}-{vm_name}"
    ...
    # Создание виртуалок
    def import_vm(self, name='', cpu=1, ram=1, template_name='', vmcx =''):
        if vmcx == '':
            vmcx = self.get_vmcx
        if template_name == '':
        ...
        vm_name=f"{self.env_prefix}-{name})"
