...
def plotHistograms(l_hist, outpath, d_opt = {}):
    ...
    if True:
        ...
        #Upper left
            ...
            leg=pad_1.BuildLegend(0.2, 0.70, 0.6, 0.95)
...
def ReformatCanvas(Pad, left_margin=-1):
    ...
    for rngobj in l_rngobj:
        yaxis = rngobj.GetYaxis()
        yaxis.SetRangeUser(scale_min, scale_max)
...
def add_df_column(rdf, arr_val, name, d_opt={}):
    ...
    l_col = [name.c_str() for name in v_col]
    data  = ak.from_rdataframe(rdf, columns=l_col)
    d_data= { col : data[col] for col in l_col }

    #Because cannot save long unsigned
    awk_evt = d_data['eventNumber']
    arr_evt = awk_evt.to_numpy()
    arr_evt = arr_evt.astype(float)
    del(d_data['eventNumber'])
    d_data['feventNumber']  = arr_evt

    d_data[name]   = arr_val

    rdf = ak.to_rdataframe(d_data)
    rdf = rdf.Define('eventNumber', '(unsigned)feventNumber')
    rdf = mgr.add_atr(rdf)
