...
class CeleryClient(object):
    ...
    # 异步请求
    def apply_async(self, task_name: str, kwargs: Dict = None, callback: Callback = None):
        ...
        try:
            ...
            # 异步调用
            async_result = self._get_task(task_name).apply_async(kwargs=kwargs, queue=f'task_queue.{task_name}', headers=_headers)
            # 绑定回调处理函数
            self._bind_callback(task_name, async_result, callback)
            return async_result
        except Exception as e:
            LOGGER.error(f'处理异常: task_name={task_name}, kwargs={kwargs}, callback={callback}, e={e}')
            raise e

    # 同步请求
    def apply_sync(self, task_name: str, kwargs: Dict = None, timeout: float = None):
        ...
        try:
            ...
            LOGGER.error(f'处理异常: task_name={task_name}, kwargs={kwargs}, e={e}')
            raise e

    # 兼容方式请求调用
    def apply(self, task_name: str, kwargs: Dict = None, callback: Callback = None, timeout: float = None):
        """
        兼容方式请求调用
        有callback怎为异步
        """
        if callback:
            # 异步
            return self.apply_async(task_name=task_name, kwargs=kwargs, callback=callback)
        else:
            # 同步
            return self.apply_sync(task_name=task_name, kwargs=kwargs, timeout=timeout)
