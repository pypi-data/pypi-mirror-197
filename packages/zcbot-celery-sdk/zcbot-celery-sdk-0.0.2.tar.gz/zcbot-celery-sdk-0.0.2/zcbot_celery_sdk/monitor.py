...
from .multi_thread import run_in_worker
from .utils import ref_to_obj

LOGGER = logging.getLogger(__name__)


class CeleryResultMonitor(object):
    ...
    def start(self):
        ...
        Thread(target=self._run, name='monitor').start()
                ...
                for key in keys:
                    ...
                    if task_id:
                        ...
                        if async_result.successful():
                            ...
                            callback = json.loads(self.rds_client.getdel(key))
                            callback_func = callback.get('callback_func', None)
                            callback_data = callback.get('callback_data', None)
                            if callback_func:
                                func = ref_to_obj(callback_func)
                                await run_in_worker(func, result, callback_data)
                                LOGGER.info(f'结果: task={task_id}, func={callback_func}')
                            else:
                                LOGGER.info(f'未处理: task={task_id}')
