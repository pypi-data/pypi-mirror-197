...
class StreamBotAPI:
    def __init__(self, streambots, host='0.0.0.0', port=80, origins=['*'], verbosity = 0, log_file=None, debug=False):
        ...
        self.socketio = SocketIO(self.app, async_mode="eventlet", cors_allowed_origins="*", websocket=True, log_output=False)
        self.init_cors()
        self.init_routes()
    ...
    def handle_messages(self):
        ...
        def emit_callback():
            ...
            if self.verbosity >= 1:
                self.logger.info(f'{connection_id} message array: {self.messages[connection_id]}')

            return jsonify(self.messages[connection_id])
        
        self.socketio.emit('emit_event', callback=emit_callback)
        return 'OK', 200
    ...
    def start(self):
        ...
        self.socketio.run(self.app, host=self.host, port=self.port, debug=self.debug, log_output=False)
