...
def upload_to_r2(title, bucket='podcast'):
    s3 = boto3.resource(
        's3',
        endpoint_url = set_env_var('R2_ENDPOINT_URL'),
        aws_access_key_id = set_env_var('R2_ACCESS_KEY_ID'),
        aws_secret_access_key = set_env_var('R2_SECRET_ACCESS_KEY'),
    )

    filetypes = '(.+\.mp3|index.rss)' #regex pattern in string
    files = collect_files(filetypes)
    podcastbucket = create_bucket(bucket, s3)
