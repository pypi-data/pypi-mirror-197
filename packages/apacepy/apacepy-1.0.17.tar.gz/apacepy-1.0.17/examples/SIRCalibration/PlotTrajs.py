...
def plot(prev_multiplier=52, incd_multiplier=1, obs_incd_multiplier=1, filename='trajectories.png'):
    ...
    ObsIncRate = A.TrajPlotInfo(outcome_name='Obs: Incidence rate',
                                title='Observed Incidence Rate',
                                x_multiplier=obs_incd_multiplier,
                                calibration_info=A.CalibrationTargetPlotInfo(
                                    rows_of_data=D.IncidenceRate, if_connect_obss=True))

    sim_outcomes.plot_multi_panel(n_rows=2, n_cols=3,
                                  list_plot_info=[S, I, R, Inc, ObsInc, ObsIncRate],
