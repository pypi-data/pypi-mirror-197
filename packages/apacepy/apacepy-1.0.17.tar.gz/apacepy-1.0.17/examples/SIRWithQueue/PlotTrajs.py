...
def plot(prev_multiplier=52, incd_multiplier=1):
    ...
    R = A.TrajPlotInfo(outcome_name='In: Recovered',
                       title='Recovered',
                       x_multiplier=prev_multiplier)
    Q = A.TrajPlotInfo(outcome_name='In: Hospital queue',
                       title='Hospital Queue',
                       x_multiplier=prev_multiplier)
    H = A.TrajPlotInfo(outcome_name='In: Hospitalized',
                       title='Hospitalized',
                       x_multiplier=prev_multiplier)
    Inc = A.TrajPlotInfo(outcome_name='To: Infectious',
                         title='Incidence',
                         x_multiplier=incd_multiplier)
    new_hospt = A.TrajPlotInfo(outcome_name='To: Hospitalized',
                               title='New hospitalizations',
                               x_multiplier=incd_multiplier)
    new_deaths = A.TrajPlotInfo(outcome_name='To: Death',
                                title='Deaths',
                                x_multiplier=incd_multiplier)
    total_deaths = A.TrajPlotInfo(outcome_name='Total to: Death',
                                  title='Total Deaths',
                                  x_multiplier=prev_multiplier)

    sim_outcomes.plot_multi_panel(n_rows=3, n_cols=3,
                                  list_plot_info=[S, I, R, Q, H, total_deaths, Inc, new_hospt, new_deaths],
                                  file_name='figures/SIRWithQueue.png')
