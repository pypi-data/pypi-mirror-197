...
from .util import xml_to_dict, pred_rt, calculate_memory_usage
from .config import *
...
def parse_args() -> argparse.Namespace:
    ...
    _parser.add_argument('--rt_alignment_percentile', required=False, default=95, type=int,help=RT_ALIGNMENT_PERCENTILE_DESCRIPTION)
    _parser.add_argument('--rt_score', required=False, default=False, action=argparse.BooleanOptionalAction, help=RT_ALIGNMENT_PERCENTILE_DESCRIPTION)
    _parser.add_argument('--mod_map', required=False, default='{}', type=str, help=MODIFICATION_MAP_DESCRIPTION)
...
def run():
    ...
    alignment_figs, pin_df, dta_filter_content = mokafilter((sqt_ios, sqt_stems), (fasta_ios, fasta_stems),
                                                            ...
                                                            args.filter_level,
                                                            args.rt_alignment_percentile, args.rt_score,
                                                            ast.literal_eval(args.mod_map)
    )
...
def mokafilter(sqts: Tuple[Iterator[IO[str]], List[str]],
               ...
               filter_level: int,
               rt_alignment_percentile: int,
               rt_score: bool,
               mod_map: dict) -> (List[matplotlib.figure.Figure], pd.DataFrame, IO[str]):
    ...
    if rt_score is True:
        print(f'Scoring RT')
        _ = [pred_rt(sqt_df, mod_map, rt_alignment_percentile) for sqt_df in sqt_dfs]

    sqt_df = pd.concat(sqt_dfs, ignore_index=True)
    sqt_df['locuses'] = sqt_df['locuses'].astype("category")
    sqt_df['file'] = sqt_df['file'].astype("category")
    sqt_df['validation_status'] = sqt_df['validation_status'].astype("category")

    print(sqt_df.dtypes, end="\n\n")
    print(calculate_memory_usage(sqt_df))

    pin_df = convert_to_moka(sqt_df)

    print(pin_df.dtypes, end="\n\n")
    print(calculate_memory_usage(pin_df))
