...
class OracleTools:
    ...
    def desc(ora_conn, username, table):
        """ 输出一个表的，列名、字段类型、主键、索引（索引所在的列而非索引名）和默认值 """
        ...
        sql = '''
            select column_name, 
                case 
                    ...
                    when data_type = 'NVARCHAR' or data_type = 'NVARCHAR2' then 
        ...
        sql = '''
            ...
            and t.index_type in('FUNCTION-BASED NORMAL', 'NORMAL')
        ...
        rss = ora_conn.run(sql, (username, table)).get_rows()
        for rs in rss:
            mess = 'data_default: ' + rs[0] + ' ' + rs[1].replace('  /* GOLDENGATE_DDL_REPLICATION */', '').strip().strip('\n')
            tab = tab + mess + '\n'
    ...
    def get_ddl(dba_conn, username, table):
        """ 得到目标user.table的单元性的建表语句，包括按顺序的列名、列类型、默认值、非空约束，普通索引、函数索引和注释 """
        """ 不包括外键、分区信息、存储信息和特殊索引类型 """
        username = username.upper()
        table = table.upper()
        rssqls = []
        # 建表
        ...
        '''
        for r in dba_conn.run(sql, (username, table)).get_rows():
            rssqls.append('alter table ' + username + '.' + table + ' modify ' + r[0] + ' default ' + r[1].strip().strip('\n'))
        # 建索引，包括普通索引和普通函数索引
        sql = '''
            select t.index_name
            , t.index_type
        ...
        if len(rows) > 0:
            col = dba_conn.run(sql, (username, table)).get_cols()
            mdb = MemoryDB()
            mdb.set('t_idx', col, rows)
            # 降序索引也体现为函数索引，列名会用"col_name"，而字符串用'str'
            sql = '''
                ...
                from t_idx 
            '''
            rs = mdb.db_conn.run(sql).get_rows()
            mdb.close()
            for r in rs:
