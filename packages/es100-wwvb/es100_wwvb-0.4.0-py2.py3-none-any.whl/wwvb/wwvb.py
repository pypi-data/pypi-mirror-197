...
from .driver28client import Driver28Client, Driver28ClientError

# ES100's pins as connected to R.Pi GPIO pins

                                        # ES100 Pin 1 == VCC 3.6V (2.0-3.6V recommended)
GPIO_IRQ  = 11 # GPIO-17                # ES100 Pin 2 == IRQ Interrupt Request
                                        # ES100 Pin 3 == SCL
                                        # ES100 Pin 4 == SDA
...
def doit(program_name, args):
    ...
    try:
        es100 = ES100(antenna=antenna_choice, en=es100_en, irq=es100_irq, debug=flag_debug, verbose=flag_verbose)
    ...
    if ntpd_unit_number is not None:
        try:
            driver28 = Driver28Client(unit=ntpd_unit_number, debug=flag_debug, verbose=flag_verbose)
        except Driver28ClientError as err:
            sys.exit(err)
...
def receive(es100, log, flag_force_tracking, flag_enable_nighttime, my_location, my_masl):
    ...
    :param my_location [lat, lon]: Receivers location. Negative lat and lon is South and West.
    :param my_masl: Receivers MASL (Meters Above Sea Level)
