...
from zod.eval.detection._nuscenes_eval.detection.constants import DETECTION_NAMES
...
if __name__ == "__main__":
    ...
    zod = ZodFrames("/Users/s0000960/data/zod", "mini")
    gt_boxes = EvalBoxes()
    for sample in zod:
        od_annos = sample.get_annotation(AnnotationProject.OBJECT_DETECTION)
        boxes = []
        for obj in od_annos:
            if obj.name not in DETECTION_NAMES:
                continue
            if obj.box3d is None:
                continue
            boxes.append(
                DetectionBox(
                    sample_token=sample.info.id,
        ...
        gt_boxes.add_boxes(sample.info.id, boxes)
    evaluate_nuscenes_style(gt_boxes, gt_boxes, verbose=True, verify_coordinate_system=False)
