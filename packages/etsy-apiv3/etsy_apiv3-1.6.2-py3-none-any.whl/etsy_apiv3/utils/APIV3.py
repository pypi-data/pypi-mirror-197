...
class EtsySession:
    ...
    def __init__(self,
        ...
        if token is not None:
            ...
            self.__async_session = self.__create_async_oauth2_session()
            self.__me = Me(**self.request(f"users/me"))

        else:
            self.__oauth2_session = requests.Session()
            self.__async_session = AsyncClient()
    ...
    def __create_async_oauth2_session(self) -> AsyncOAuth2Session:
        ...
        session = AsyncOAuth2Session(
            self.CLIENT_KEY, token=self.TOKEN, auto_refresh_kwargs=refresh_kwargs,
            auto_refresh_url=self.__refresh_url, token_updater=self.token_updater
        )
        return session
    ...
    async def async_request(self, endpoint: str, method="GET", headers: Optional[dict] = None, *args, **kwargs) -> dict:
        ...
        req = await self.__async_session.request(method=method, url=url, headers=headers, *args, **kwargs)
        json = await req.json()
