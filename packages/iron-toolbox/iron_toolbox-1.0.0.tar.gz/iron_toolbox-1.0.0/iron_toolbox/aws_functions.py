...
def create_table_to_update(database, bucket, folder_name, table, update_table, col_update, col_drop,
    ...
    try:
        ...
        dt_ultimo_registro = str(get_max_updated_date_from_table(database, table, col_update))[0:23]
        # table_to_update = pd.DataFrame()
        print(f'Tabela para update vazia, error: {e}')
        print(dt_ultimo_registro)
    table_to_update = pd.DataFrame()
    ...
    table_lenght = len(table_to_update)
    if table_lenght > 173000:
        mode_update = 'overwrite'
    else:
        mode_update = 'append'
    ...
    return table_lenght, dt_ultimo_registro, table_to_update, data_to_update,
